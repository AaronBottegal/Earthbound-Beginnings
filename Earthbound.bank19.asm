LOOPY_IDFK: ; 13:0000, 0x026000
    LDA #$05
    STA SOUND_FX_REQUEST_B? ; Request cursor movement sound.
...
VAL_LT_0x4: ; 13:0023, 0x026023
    ...
    JSR CREATE_PTR_FOR_SAVE/ROM_PAGE_UNK ; Do ptr.
    TXA ; X to A.
    PHA ; Save it.
...
VAL_LT_0x2C: ; 13:003B, 0x02603B
    ...
    JSR ENGINE_MENU_INIT_PRIMARY ; Do.
...
SUB_PAGE_AND_FILE_MOVE_UNK: ; 13:008F, 0x02608F
    ...
    TXA
    PHA
    JSR R6_BANK_AND_SETUP_PTR_9800_FROM_VAR ; Do.
    LDY #$00 ; Stream reset.
...
ENGINE_UNK_OBJECTS_REELATED?: ; 13:0178, 0x026178
    ...
    STA FPTR_MENU_MASTER[2]
    STX FPTR_MENU_MASTER+1
    JSR ENGINE_MENU_INIT_PRIMARY ; Do, menu?
    BIT SCRIPT_MENU_STATUS ; Test.
...
RTN_0x1: ; 13:0262, 0x026262
    ...
    LDA #$D1
    LDX #$F0
    STA FPTR_MENU_SUBMENUUUUUUUUUUUUU[2]
    STX FPTR_MENU_SUBMENUUUUUUUUUUUUU+1
    LDA #$9A
    LDX #$A2
    STA FPTR_MENU_MASTER[2]
    STX FPTR_MENU_MASTER+1
    ...
    .db 00
    LDA #$FF
    JSR ENGINE_CURSOR_TO_BG_UPDATE_INTO_MENU?
    JSR 13:092D
...
PTR_AND_CREATE_UNK: ; 13:0445, 0x026445
    ...
    BNE 13:0544
    LDX #$2C
    STX FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]
    JSR $A912
    LDX FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]
    JSR $AA7C
    JSR $AA4E
    BCS 13:056A
    JSR $A964
    ...
    BCS 13:06D0
    LDA R_**:$002A
    ADC LOCAL_VARS_ARR?[8]
    STA R_**:$002A
    LDA R_**:$002B
    ADC LOCAL_VARS_ARR?+1
    STA R_**:$002B
    RTS
    CLC
    LDA [GFX_BANKS[4]],Y
...
ALT_RTN_UNK: ; 13:0909, 0x026909
    ...
    LDA R_**:$0028
    JSR CREATE_PTR_FOR_SAVE/ROM_PAGE_UNK
    LDA BCD/MODULO/DIGITS_USE_A
    STA GFX_BANKS[4]
    LDA BCD/MODULO/DIGITS_USE_B
    STA GFX_BANKS+1
...
ENGINE_RESET_GAME: ; 13:0C4B, 0x026C4B
    ...
    BNE OPT_B_EXIT_RESTREAM_FROM_NEXT_STREAM
    TXA
    LSR A
    CMP **:$0034
    BNE OPT_B_EXIT_RESTREAM_FROM_NEXT_STREAM
...
VAL_EQ_0x00: ; 13:0D61, 0x026D61
    ...
    STA R_**:$0070 ; Clear ??
    STA ENGINE_PACKINATOR_ARG_SEED_0xA0_PRE_COUNT ; Clear ??
    LDY STREAM_INDEX_FILE_UNK_SCRIPT/COMMANDS_TEXT? ; Load index.
...
MENU_HELPER_UNK_TODO: ; 13:0DC5, 0x026DC5
    ...
    LDX #$F0
    STA FPTR_MENU_SUBMENUUUUUUUUUUUUU[2] ; Store submenu.
    STX FPTR_MENU_SUBMENUUUUUUUUUUUUU+1
...
0x40_SET: ; 13:0E20, 0x026E20
    JMP OPT_B_EXIT_RESTREAM_FROM_NEXT_STREAM ; Goto.
    JSR ENGINE_WRAM_STATE_WRITEABLE
    JSR L_13:0E58
    ORA LUT_INDEX_TO_BITS_0x80-0x01,X
...
L_13:0F12: ; 13:0F12, 0x026F12
    JMP OPT_B_EXIT_RESTREAM_FROM_NEXT_STREAM
    CLC
...
L_13:10A3: ; 13:10A3, 0x0270A3
    JSR R6_BANK_AND_SETUP_PTR_9800_FROM_VAR
    LDY #$02
    LDA [BCD/MODULO/DIGITS_USE_A],Y
    AND #$80
    RTS
...
STREAM_STORE_AND_SCRIPT_PTR_FROM_VAR: ; 13:15C2, 0x0275C2
    STY STREAM_INDEX_FILE_UNK_SCRIPT/COMMANDS_TEXT? ; Store index.
SUB_TODO: ; 13:15C4, 0x0275C4
    LDA R_**:$0028 ; Load val.
    JMP CREATE_PTR_FOR_SAVE/ROM_PAGE_UNK ; Create PTR.
...
L_13:17B6: ; 13:17B6, 0x0277B6
    ...
    LDA [FPTR_MENU_SUBMENUUUUUUUUUUUUU[2]],Y
    STA PTR_CREATE_SEED_UNK
    CLC
    RTS
    JSR $B5C4
...
L_13:187F: ; 13:187F, 0x02787F
    ...
    LDY #$00
    STX GFX_COORD_HORIZONTAL_OFFSET
    STY MENU_SELECTED_SUBMENU_OPTION_INDEX_FINAL
    LDA [FPTR_MENU_SUBMENUUUUUUUUUUUUU[2]],Y
    STA PTR_CREATE_SEED_UNK
    JSR $BBAF
    LDX #$13
    CPX GFX_COORD_HORIZONTAL_OFFSET
    BNE 13:1B63
...
PTR_MOVE_TO_UNK_ARR_WRAM: ; 13:1B8C, 0x027B8C
    JSR R6_BANK_AND_SETUP_PTR_9800_FROM_VAR ; Get ptr.
    LDY #$00 ; Stream index.
    LDA [BCD/MODULO/DIGITS_USE_A],Y ; Load from stream.
...
PTR_AND_BANK_R6_UNK: ; 13:1BC3, 0x027BC3
    JSR R6_BANK_AND_SETUP_PTR_9800_FROM_VAR ; Do ptr.
    LDY #$06 ; Stream index.
...
R6_AND_FILE_MANIP_UNK: ; 13:1BD4, 0x027BD4
    JSR R6_BANK_AND_SETUP_PTR_9800_FROM_VAR ; Do ptrs.
    LDY #$02 ; R6 index.
    JSR MISC_FILE_MANIP_UNK ; Do ??
    JMP EXIT_STREAM_DATA_AS_R6_ROUTINE ; Goto.
R6_BANK_AND_SETUP_PTR_9800_FROM_VAR: ; 13:1BDF, 0x027BDF
    JSR SUB_PTR_AND_R6_SWAP ; Do sub.
SETUP_PTR_9800_UNK: ; 13:1BE2, 0x027BE2
    CLC ; Prep add.
...
SUB_PTR_AND_R6_SWAP_SEEDED: ; 13:1BF2, 0x027BF2
...
PACKETY_HELPER_UNK: ; 13:1C0A, 0x027C0A
    ...
    LDX #$08
    LDY #$13
    STX GFX_COORD_HORIZONTAL_OFFSET ; Set POS.
    STY GFX_COORD_VERTICAL_OFFSET
    JMP EXIT_STREAM_DATA_AS_R6_ROUTINE ; Goto.
...
ADD_NO_OVERFLOW_EXTEND: ; 13:1C5E, 0x027C5E
    ...
    STA [LOCAL_VARS_ARR?[8]],Y ; Store to file.
    INY ; Stream++
    LDA [LOCAL_VARS_ARR?[8]],Y ; Load from file.
    SBC #$00 ; Carry sub.
    STA [LOCAL_VARS_ARR?[8]],Y ; Store to file.
    ...
    CLC ; Prep add.
    LDA [LOCAL_VARS_ARR?[8]],Y ; Load from file.
    ADC LOCAL_VARS_ARR?+3 ; Add with.
    STA [LOCAL_VARS_ARR?[8]],Y ; Store back.
    INY ; Stream++
    LDA [LOCAL_VARS_ARR?[8]],Y ; Load from file.
    ADC #$00 ; Carry add.
    STA [LOCAL_VARS_ARR?[8]],Y ; Store to file.
...
VALUE_NEGATIVE: ; 13:1CA6, 0x027CA6
    ...
    ASL A ; << 1, *2.
    EOR #$FF ; Invert.
    AND [LOCAL_VARS_ARR?[8]],Y ; Mask with file.
    STA [LOCAL_VARS_ARR?[8]],Y ; Store to file.
    LDA LOCAL_VARS_ARR?+3 ; Load ??
    ASL A ; << 1, *2.
    ORA [LOCAL_VARS_ARR?[8]],Y ; Set bits with file.
    STA [LOCAL_VARS_ARR?[8]],Y ; Store back to file.
EXIT_UNK: ; 13:1CB8, 0x027CB8
    LDA LOCAL_VARS_ARR?+3 ; Load ??
    BEQ VAL_EQ_0x00 ; == 0, goto.
    LDY LOCAL_VARS_ARR?+2 ; Stream index from.
    LDA [LOCAL_VARS_ARR?[8]],Y ; Load from file.
ROM_DATA_USE_UNK: ; 13:1CC0, 0x027CC0
    ...
    LDA [LOCAL_VARS_ARR?[8]],Y ; Load from file.
    BNE VAL_NONZERO
    LDY LOCAL_VARS_ARR?+2 ; Load file index.
    BNE VAL_NONZERO_B ; != 0, goto.
VAL_LT_0x28: ; 13:1CCB, 0x027CCB
    LDA [LOCAL_VARS_ARR?[8]],Y ; Load from file.
    DEY ; Stream--
    STA [LOCAL_VARS_ARR?[8]],Y ; Store to file, moved down.
    INY ; Stream++
...
VAL_NONZERO: ; 13:1CDB, 0x027CDB
    LDY LOCAL_VARS_ARR?+2 ; Load stream index.
...
PALETTE_FADE_WITH_??: ; 13:1D0D, 0x027D0D
    ...
    LDA #$10
    STA SOUND_FX_REQUEST_B? ; Set sound effect.
    LDA #$34 ; Palette data.
...
MOVE_PAGE_DATA: ; 13:1E92, 0x027E92
    LDA [LOCAL_VARS_ARR?[8]],Y ; Take game file data.
    STA [SAVE_GAME_MOD_PAGE_PTR[2]],Y ; Store to mod page.
    INY ; Stream++
    BNE MOVE_PAGE_DATA ; != 0, loop.
    INC LOCAL_VARS_ARR?+1 ; Inc PTR H for both ptrs.
...
SAVEGAME_INIT_BASE_FPTR_FOR_FILE_A: ; 13:1EBB, 0x027EBB
    ...
    ASL A ; << 1, *2. CC now.
    ADC LOCAL_VARS_ARR?+1 ; Add with, *3. Slot size.
    ADC #$77 ; Addr + 0x7700, base of saved files.
    STA LOCAL_VARS_ARR?+1 ; Store ptr H, 0x77/0x7A/0x7D.
    LDA #$00
    STA LOCAL_VARS_ARR?[8] ; Clear addr L.
