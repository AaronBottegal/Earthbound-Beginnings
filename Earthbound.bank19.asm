...
STREAMY_LIBY_UNK: ; 13:01C6, 0x0261C6
    ...
    BEQ RTS ; == 0, goto.
    LDA #$35
    STA SCRIPT_COMPARE_TODO_UNK ; Set ??
...
RTN_B: ; 13:0451, 0x026451
    ...
    LDA #$0D
    STA SCRIPT_COMPARE_TODO_UNK ; Clear ??
    JSR STREAM_PROCESS_UNK_INIT ; Do streams.
...
SCRIPT_CREATE_SAVE_PTR_TODO: ; 13:06A5, 0x0266A5
    ...
    LDA [GFX_BANKS_EXTENSION[4]],Y ; Load from file.
    ADC PAIR_UNK_LEVEL_UP_RELATED?[2] ; Add with ??
    STA SAVE_GAME_MOD_PAGE_PTR[2] ; Store to save ptr.
    INY ; Stream++
    LDA [GFX_BANKS_EXTENSION[4]],Y ; Load ??
    ADC PAIR_UNK_LEVEL_UP_RELATED?+1 ; Add with ??
    STA SAVE_GAME_MOD_PAGE_PTR+1 ; Store to mod ptr.
    RTS ; Leave.
...
SUB_A_TO_UNK_SCRIPT_SAVE_STUFF_AND_RESTORE: ; 13:0912, 0x026912
    STA BCD/MODULO/DIGITS_USE_A ; Store ??
    LDA #$00
    STA BCD/MODULO/DIGITS_USE_B ; Clear upper.
    JSR SCRIPT_HARD_SWITCH_TO_SOMETHING_HUGE ; Do sub, hard swap unk. TODO.
...
EXIT_STREAM_HEADER_AS_R6_ROUTINE: ; 13:0B41, 0x026B41
...
SYNC_UNK: ; 13:0C6D, 0x026C6D
    ...
    TXA ; X to A.
    LSR A ; >> 1, /2.
    CMP SCRIPT_COMPARE_TODO_UNK ; If _ var
    BNE MAIN_SCRIPT_RESTREAM_SELF ; !=, goto.
...
SCRIPT_UNK_A: ; 13:0CBA, 0x026CBA
    ...
    JSR ENGINE_PALETTE_SCRIPT_TO_TARGET ; Do palette to target.
    INY ; Stream++
    LDA [SCRIPT_MAIN_FPTR[2]],Y ; Load from file.
...
VAL_NONZERO: ; 13:0D40, 0x026D40
    ...
    JSR SUB_MOD_PACKETS_TODO_UNK ; Mod packets.
    CMP #$00 ; If _ #$00
    BEQ VAL_EQ_0x00 ; == 0, goto.
    LDY #$00 ; Stream index.
...
VAL_EQ_0x00: ; 13:0D61, 0x026D61
    JSR EXIT_STREAM_HEADER_AS_R6_ROUTINE ; Do routine.
    LDA #$00
    STA R_**:$0070 ; Clear ??
...
MENU_HELPER_UNK_TODO: ; 13:0DC5, 0x026DC5
    ...
    LDA MENU_SELECTED_SUBMENU_OPTION_INDEX_FINAL ; Load ??
    BNE EXIT_RESTREAM_FROM_STREAM ; != 0, goto.
    INY ; Stream++
    INY
    RTS ; Leave.
...
ADD_NO_OVERFLOW: ; 13:0F47, 0x026F47
    JSR ENGINE_WRAM_STATE_WRITEABLE ; Do WRAM.
    LDA BCD/MODULO/DIGITS_USE_A ; Move result ??
    ...
    JSR LIB_DECIMAL?_UNK ; Do lib.
    LDA #$64
    STA SAVE_GAME_MOD_PAGE_PTR[2] ; Set mod PTR.
...
STREAMS_A_FEW_OPTIONS_TODO: ; 13:0FC4, 0x026FC4
    ...
    STY SCRIPT_MAIN_FILE_STREAM_INDEX ; Strea save.
    LDA #$00 ; Load ??
    JSR TEST_FOR_VAL_EQ_PASSED ; Do ??
    BCS EXIT_STREAM_RESTREAM_SELF ; CS, goto.
    BCC EXIT_ALT_A ; Other, always taken.
    STY SCRIPT_MAIN_FILE_STREAM_INDEX ; Store stream index.
    JSR SUB_GET_UNK_NEGATIVE_BIT_ONLY ; Get ??
    PHP ; Save status.
    JSR EXIT_STREAM_HEADER_AS_R6_ROUTINE ; Do other sub.
    PLP ; Pull status.
...
RESTORE_COORDS_AND_TODO: ; 13:11A5, 0x0271A5
    ...
    PLA
    STA GFX_COORD_HORIZONTAL_OFFSET
    BCS EXIT_RESTREAM_MAIN_SELF ; CS, goto.
    JSR PTR_AND_BANK_R6_UNK ; Do.
    JSR PTR_MOVE_TO_UNK_ARR_WRAM ; Do.
EXIT_SCRIPT_INC_0x2: ; 13:11B3, 0x0271B3
...
VAL_LT_0x4_LOOP: ; 13:11C1, 0x0271C1
    ...
    PLA ; Restore X.
    TAX
    BCC EXIT_SCRIPT_INC_0x2 ; CC, exit.
SLOT_INVALID: ; 13:11D1, 0x0271D1
    ...
    BCC EXIT_SCRIPT_INC_0x2 ; CC, Inc 2.
...
SET_SAVE_PAGE_PTR_L?: ; 13:11ED, 0x0271ED
    STY SAVE_GAME_MOD_PAGE_PTR[2] ; Store PTR L.
    LDY #$00 ; Stream index.
...
EXIT_RET_CC: ; 13:11FB, 0x0271FB
    ...
    STY BCD/MODULO/DIGITS_USE_C ; Store stream index.
    JSR SCRIPT_ENTRY_CLEAR_UNK_AND_MORE ; Do ??
    JSR EXIT_STREAM_HEADER_AS_R6_ROUTINE ; Do ??
...
EXIT_RESTREAM_SELF: ; 13:1220, 0x027220
    ...
    JSR REGISTER_FLAG_23_MANIUP_AND_STREAM_DATA_TO_UNK ; Do sub.
EXIT_STREAM_INC_WORD_AND_NO_WRAM: ; 13:1295, 0x027295
    LDY SCRIPT_MAIN_FILE_STREAM_INDEX ; Load stream.
    INY ; += 2
    INY
    JMP ENGINE_WRAM_STATE_WRITE_DISABLED ; Goto.
REGISTER_FLAG_23_MANIUP_AND_STREAM_DATA_TO_UNK: ; 13:129C, 0x02729C
    TAX ; To X index.
    STY SCRIPT_MAIN_FILE_STREAM_INDEX ; Store stream index.
    JSR ENGINE_WRAM_STATE_WRITEABLE ; Wram writable.
    TXA ; X back to A.
...
ROUTINE_ATTRIBUTE_THINGY_TODO_CONTINUE: ; 13:12DE, 0x0272DE
    ...
    RTS ; Leave.
    LDA #$74 ; Seed ??
    JSR REGISTER_FLAG_23_MANIUP_AND_STREAM_DATA_TO_UNK
    LDA #$09 ; Seed ??
    LDX #$FC
    LDY #$8A
    ...
    JMP EXIT_STREAM_INC_WORD_AND_NO_WRAM ; Exit.
    JSR ENGINE_WRAM_STATE_WRITEABLE ; WRAM writable.
    LDA #$F8
    STA WRAM_UNK ; Set ??
    INY ; Stream++
    ...
    LDA #$74 ; Seed ??
    JSR REGISTER_FLAG_23_MANIUP_AND_STREAM_DATA_TO_UNK ; Do.
    LDA #$0A ; Seed ??
    LDX #$1C
    LDY #$8B
    ...
    JMP EXIT_STREAM_INC_WORD_AND_NO_WRAM ; Goto, return.
    LDA #$74
    JSR REGISTER_FLAG_23_MANIUP_AND_STREAM_DATA_TO_UNK ; Seed ??
    LDA #$0B ; Seed ??
    LDX #$3C
    LDY #$8B
    JSR ROUTINE_ATTRIBUTE_THINGY_TODO_INIT ; Do.
...
STREAM_WRAM_PAIR_UNK_AND_RTN_ATTRS: ; 13:138B, 0x02738B
    ...
    JMP OPTIONS_CONTINUE_FILE_NO_RESTREAM_ON_EQ ; Exit restream/continue.
    STY SCRIPT_MAIN_FILE_STREAM_INDEX ; Store index.
    LDY #$00 ; Stream reset.
    LDA [SCRIPT_MAIN_FPTR[2]],Y ; Load ??
    AND #$C0 ; Keep upper.
...
EXIT_REINDEX_SELF: ; 13:141B, 0x02741B
    ...
    INY ; Stream++
    LDA [SCRIPT_MAIN_FPTR[2]],Y ; Load from file.
    STA SCRIPT_ENCOUNTER_ID?(SAID_SONG_ID???) ; Store to.
    ...
    SBC [BCD/MODULO/DIGITS_USE_A],Y ; Sub with file.
    STA PAIR_UNK_LEVEL_UP_RELATED?[2] ; Store to.
    INY ; Stream++
...
LOOP_INDEX_LT_0x4: ; 13:151D, 0x02751D
    ...
    JSR CREATE_PTR_FROM_PASSED_TO_BANKED/WRAM_DATA ; Do WRAM.
    LDY #$01 ; Stream index.
    LDA [BCD/MODULO/DIGITS_USE_A],Y ; Load ??
    ...
    ADC PAIR_UNK_LEVEL_UP_RELATED?+1 ; Add with.
    STA PAIR_UNK_LEVEL_UP_RELATED?+1 ; Store to. ??
    BCC SLOT_INVALID ; No overflow, goto.
...
STREAM_MOVE_INSIDE_FILE_0x5->0x16_0x6->0x17: ; 13:1598, 0x027598
    ...
    STA [BCD/MODULO/DIGITS_USE_A],Y
    RTS
    JSR STREAM_STORE_AND_SCRIPT_PTR_FROM_VAR ; Stream store and ID from ??
    SEC ; Prep sub.
    LDY #$14 ; File index.
    LDA [BCD/MODULO/DIGITS_USE_A],Y ; Load ??
    LDY #$03
    ...
    LDA [BCD/MODULO/DIGITS_USE_A],Y ; Load ??
    LDY #$04
    SBC [BCD/MODULO/DIGITS_USE_A],Y ; Sub with, carry sub.
...
SUB_DATA_PTR_FROM_RAM[0x28]: ; 13:15C4, 0x0275C4
    ...
    LDY SCRIPT_MAIN_FILE_STREAM_INDEX
    AND [SCRIPT_MAIN_FPTR[2]],Y
    JMP OPTIONS_CONTINUE_FILE_NO_RESTREAM_ON_EQ
    INY
    JSR STREAM_STORE_AND_SCRIPT_PTR_FROM_VAR
    LDY #$10
    LDA [BCD/MODULO/DIGITS_USE_A],Y
    ...
    JMP OPTIONS_CONTINUE_FILE_NO_RESTREAM_ON_CS
    INY
    JSR STREAM_STORE_AND_SCRIPT_PTR_FROM_VAR
    ...
    LDY #$A5
    JSR ENGINE_SCRIPT_LAUNCH_R7_A_PTR_XY_WITH_RESTORE
    JSR EXIT_STREAM_HEADER_AS_R6_ROUTINE
    LDY SCRIPT_MAIN_FILE_STREAM_INDEX
    INY
    RTS
    STY SCRIPT_MAIN_FILE_STREAM_INDEX
...
VAL_CLEAR: ; 13:177E, 0x02777E
    ...
    JSR EXIT_STREAM_HEADER_AS_R6_ROUTINE ; Do file from var.
    LDA #$AC
    LDX #$B7
...
SUB_SLOT_TO_SUBMENU_TODO: ; 13:18CA, 0x0278CA
    ...
    ADC #$B0 ; += 0xB0
    STA FPTR_MENU_SUBMENU[2] ; Store as submenu L.
    LDA #$00 ; Seed carry add.
    ADC #$76 ; Add with val.
    STA FPTR_MENU_SUBMENU+1 ; Store as submenu H.
    RTS
    .db 20
    .db 09
    .db 03
...
SHIFT_CC: ; 13:19C0, 0x0279C0
    ...
    STA SPRITE_PAGE+4
    JSR ENGINE_WRAM_STATE_WRITE_DISABLED
    JMP EXIT_STREAM_HEADER_AS_R6_ROUTINE
    LDY #$11
    LDA 13:1AB9,X
...
A_NONZERO: ; 13:1B9F, 0x027B9F
    ...
    JSR ENGINE_WRAM_STATE_WRITE_DISABLED ; Do disabled.
    JMP EXIT_STREAM_HEADER_AS_R6_ROUTINE ; Exit.
...
R6_AND_FILE_MANIP_UNK: ; 13:1BD4, 0x027BD4
    ...
    JMP EXIT_STREAM_HEADER_AS_R6_ROUTINE ; Goto.
...
SUB_COORD_SAVE_AND_DO_UNK: ; 13:1C28, 0x027C28
    ...
    LDA GFX_COORD_VERTICAL_OFFSET
    PHA
    JSR PTR_SEEDED_UNK_H_GFX_UPDATE ; Create packet?
    PLA
