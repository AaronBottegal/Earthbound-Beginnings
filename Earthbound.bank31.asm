...
VAL_LT_0xE: ; 1F:0024, 0x03E024
    LDA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Load from stream.
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],X ; Store ??
    INX ; Index++
    INY ; Stream++
    CPY #$0E ; If _ #$0E
    BCC VAL_LT_0xE ; << goto.
...
SPRITE_NO_HFLIP: ; 1F:0039, 0x03E039
    ...
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],X ; To ??
    INX ; Indexes++
    INY
...
LOOP_INDEXES: ; 1F:0067, 0x03E067
    LDA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],X ; Load ??
    AND #$40 ; Test ??
...
ARR_MAKE_UNK: ; 1F:0087, 0x03E087
    ...
    LDA #$00
    STA LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Clear index.
    LDX #$08 ; Seed index.
LOOP_NONZERO: ; 1F:0094, 0x03E094
    ...
    LDY LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Alt stream index.
    ...
    AND #$3F ; Keep lower.
    ASL A ; << 1, *2.
    ASL LIB_BCD/EXTRA_FILE_D ; << 1, *2.
    ROR A ; Rotate carry into A.
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],X ; Store to arr.
    LDA #$70 ; Load ??
    ASL LIB_BCD/EXTRA_FILE_D ; << 1, *2.
    ROR A ; Rotate into A.
...
FLAGGED_DATA: ; 1F:00E3, 0x03E0E3
    ...
    STA LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Store 
...
SCRIPT_RTN_A: ; 1F:01BD, 0x03E1BD
    RTS
SCRIPT_RTN_B_ELSEWHERE: ; 1F:01BE, 0x03E1BE
    LDY #$04 ; Stream index.
...
DATA_MAP_RESERVATION_HELPER?_TODO: ; 1F:02BF, 0x03E2BF
    LDA DATA_UNK_C,X ; Load ??
LIB_FILE_MOD_TODO: ; 1F:02C2, 0x03E2C2
    CLC ; Prep add.
    LDY #$16 ; Stream index.
    ...
    LDA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Load ??
    TAY ; To Y index.
    LDA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],Y ; Load ??
    AND #$3F ; Keep 0011.1111
    STA OBJ_PROCESS_COUNT_LEFT? ; Store count.
    BEQ RTS ; == 0, leave.
    LDA LIB_BCD/EXTRA_FILE_BCD_A ; Move ?? to arr spot.
    STA OBJ?_PTR?[2],Y
    ...
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Store to.
    INY ; Stream++
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; 2x
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1
    INY
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; 3x
    STA RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+2
    INY
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; 4x
...
TODO_MORE_OBJS: ; 1F:031D, 0x03E31D
    ...
    CLC ; Prep add.
    LDA [SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2]],Y ; Load.
    ...
    INY ; Stream++
    LDA [SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2]],Y ; Load from stream. TODO clarify this better in docs as bits change attr out.
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Store to. Attrs base.
    LDA RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+3 ; Seed shift.
    LSR LIB_BCD/EXTRA_FILE_BCD_A ; >> var. Test flips disable?
    BCC PALETTE_KEEP? ; CC, goto.
    LSR A ; Shift off what would be color.
    LSR A
PALETTE_KEEP?: ; 1F:0342, 0x03E342
    LSR LIB_BCD/EXTRA_FILE_BCD_A ; Shift var. Tests palette only?
...
UPPER_KEEP?: ; 1F:034A, 0x03E34A
    ...
    ASL LIB_BCD/EXTRA_FILE_BCD_A ; Shift test bits back but clear.
    ASL LIB_BCD/EXTRA_FILE_BCD_A
    ORA LIB_BCD/EXTRA_FILE_BCD_A ; Set attrs with what is left.
...
TEST_C_CLEAR: ; 1F:035D, 0x03E35D
    ADC [SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2]],Y ; Add with stream.
...
SUB_PTR_SETUP_CHECK_IDFK_GOSH: ; 1F:060E, 0x03E60E
    JSR SCRIPT_STREAM_MAIN_FPTR_REPLACE ; Do ptr setup.
    LDY #$14 ; Stream index.
    LDA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Load from stream.
    AND #$0F ; Keep lower.
    TAY ; To Y index.
...
SETUP_PTR_AND_STREAM_PAGE_VAL_TODO: ; 1F:0641, 0x03E641
    JSR SCRIPT_STREAM_MAIN_FPTR_REPLACE
...
SCRIPT_LOWER_TO_INDEX_AND_UPPER_AS_SAVE_INDEX_TO_RET: ; 1F:0646, 0x03E646
    ...
    RTS
SCRIPT_STREAM_MAIN_FPTR_REPLACE: ; 1F:0655, 0x03E655
    LDY #$02 ; Stream index.
...
STREAM_NE_0x00: ; 1F:0694, 0x03E694
    JSR SCRIPT_STREAM_MAIN_FPTR_REPLACE ; Do ptr.
    LDY #$04 ; Stream index.
    JSR LIB_MAIN_FPTR_STASH_UNK ; Do.
    JSR SUB_SWITCH_SET_0x88 ; Do ??
    SEC ; Ret CS.
    RTS
LIB_MAIN_FPTR_STASH_UNK: ; 1F:06A1, 0x03E6A1
...
MISC_FILE_MANIP_UNK: ; 1F:06A9, 0x03E6A9
    SEC ; Prep sub.
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Load from original 32
    SBC #$00 ; Sub with.
...
EXTENDED: ; 1F:06DF, 0x03E6DF
    ...
    JMP STREAM_INDEX_0x8_CLEAR_TOP ; Goto.
...
VAL_GTE_0x8: ; 1F:06FE, 0x03E6FE
    ...
    LDA #$EC ; Move ??
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y
    JSR STREAM_INDEX_0x8_CLEAR_TOP ; Do ??
    LDY #$09 ; Stream index.
    LDA #$78 ; Val ??
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Store to file.
    LDA #$00 ; Seed ??
...
SCRIPT_RTN_W: ; 1F:0720, 0x03E720
    JSR STREAM_CLEAR_UNSIGNED_INT ; Clear ??
    JSR STREAM_INDEX_0x8_CLEAR_TOP ; Do stream.
    JSR STREAM_STATIC_SET_UNK ; Do ??
    LDA #$00 ; Seed ??
...
SUB_SWITCH_SET_0x88: ; 1F:072E, 0x03E72E
    ...
    RTS ; Leave.
STREAM_CLEAR_UNSIGNED_INT: ; 1F:0733, 0x03E733
    LDA #$00 ; Clear val.
    LDY #$0C ; Stream index.
    ...
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Store to stream.
    RTS ; Leave.
STREAM_INDEX_0x8_CLEAR_TOP: ; 1F:073D, 0x03E73D
    LDY #$08 ; Stream index.
    LDA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Load from file.
    AND #$3F ; Keep 0011.1111
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Store to.
    RTS ; Leave.
STREAM_STATIC_SET_UNK: ; 1F:0746, 0x03E746
    LDY #$09 ; Stream index.
    LDA #$38 ; Val to store.
...
SCRIPT_RTN_Y: ; 1F:0756, 0x03E756
    JSR STREAM_CLEAR_UNSIGNED_INT ; LOts of small file routines. Good place to label them all.
    JSR STREAM_INDEX_0x8_CLEAR_TOP ; Clear.
    JSR STREAM_STATIC_SET_UNK ; Do static.
    JSR SCRIPT_STREAM_MAIN_FPTR_REPLACE ; Do setup.
...
SCRIPT_INIT_UNK: ; 1F:094C, 0x03E94C
    ...
    JSR STREAM_CLEAR_UNSIGNED_INT ; Clear more.
...
EXIT_MOVE_FPTR/SAVE_SWITCH?: ; 1F:09B3, 0x03E9B3
    ...
    LDY #$0C ; Stream index.
    LDA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Load from file.
    STA NMI_FP_BATTLE_UNK[2] ; Store as PTR L.
    INY ; Stream++
    LDA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Move PTR H.
    STA NMI_FP_BATTLE_UNK+1
    RTS ; Leave.
...
SCRIPT_PTR_SETUP_SUBFILE_LOWER_KEEP_ONLY: ; 1F:0A24, 0x03EA24
    JSR SCRIPT_STREAM_MAIN_FPTR_REPLACE ; Setup ptr.
    LDY #$01 ; Stream index.
    LDA [SCRIPT_MAIN_FPTR[2]],Y ; Load from file.
    AND #$40 ; Test bit.
    BEQ RTS ; Clear, leave.
...
SCRIPT_RTN_L_VERY_IMPORTANT_BANKED_HANDLES: ; 1F:0A38, 0x03EA38
    ...
    ASL A ; << 1, *2.
    TAY ; To index.
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Move from file to alt ptr.
    STA SCRIPT_MAIN_FPTR[2]
    INY ; Stream++
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Load from again.
...
VAL_NONZERO: ; 1F:0AA1, 0x03EAA1
    INY ; Stream++
    JSR LIB_MAIN_FPTR_STASH_UNK ; Do file.
    INY ; Stream++
    TYA ; To A.
    LDY #$1F ; Stream mod.
...
GET_STATE_UNK_LOWER: ; 1F:0B0B, 0x03EB0B
    ...
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y
    JSR STREAM_INDEX_0x8_CLEAR_TOP ; Remove.
    JSR STREAM_STATIC_SET_UNK ; Set ??
    LDA ROUTINE_SWITCH_ID_TODO ; Load.
...
ACTION_INVALID: ; 1F:0BD4, 0x03EBD4
    JSR DATA_INDEX_SWITCH_UNK_MOVEMENT? ; Do ??
    JSR STREAM_INDEX_0x8_CLEAR_TOP ; Stream.
    JMP EXIT_MOVE_FPTR/SAVE_SWITCH? ; Goto.
...
MOVE_ALL_POSITIVE: ; 1F:0C94, 0x03EC94
    ...
    JMP ENGINE_SET_NMI_FLAG_UPDATE_TODO_WAIT ; Wait and leave.
...
VAL_NONZERO: ; 1F:0CBC, 0x03ECBC
    JSR ENGINE_SET_NMI_FLAG_UPDATE_TODO_WAIT
    LDA CTRL_BUTTONS_PREVIOUS[2] ; Load buttons.
...
ANY_BUTTONS_PRESSED: ; 1F:0CC6, 0x03ECC6
    ...
    JMP ENGINE_SET_NMI_FLAG_UPDATE_TODO_WAIT ; Wait and abuse RTS.
    .db 78 ; GFX Banks.
    .db 00
    .db 7C
    .db 7D
...
IRQ_SCRIPT_PTRS_DEFAULT: ; 1F:0CF2, 0x03ECF2
    ...
    HIGH(SCRIPT_RTN_DEFAULT_A)
    LOW(SCRIPT_RTN_DEFAULT_C_MOVE_LATCH_MOD_BANK_WRITE_TODO)
    HIGH(SCRIPT_RTN_DEFAULT_C_MOVE_LATCH_MOD_BANK_WRITE_TODO)
    LOW(R_**:$0000) ; NULL
    HIGH(R_**:$0000)
SEED_SETTLE_0xFC_BIT_TRICKY: ; 1F:0CFC, 0x03ECFC
    LDX #$FC ; Seed ??
    .db 2C ; BIT $04A2 Opcode byte.
SEED_SETTLE_0x4_BIT_TRICKED: ; 1F:0CFF, 0x03ECFF
    LDX #$04 ; Frames to settle.
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    STX NMI_FP_BATTLE_UNK+1 ; Store X to ??
    LDX #$14 ; Seed loops.
...
ENGINE_MOVE_MAPPER_GFX_BANKS_NO_OVERRIDES: ; 1F:0D6F, 0x03ED6F
    ...
    RTS ; Leave.
SCRIPT_RTN_DEFAULT_C_MOVE_LATCH_MOD_BANK_WRITE_TODO: ; 1F:0D9B, 0x03ED9B
    ...
    STA MMC3_BANK_DATA
    RTS ; Leave.
LIB_SEED_PPU_ADDR_0x24/0x1F_HELPER: ; 1F:0DCB, 0x03EDCB
...
ENGINE_PALETTE_FADE_OUT: ; 1F:0DDC, 0x03EDDC
    JSR ENGINE_PALETTE_COMMIT_TO_SCRIPT ; Settle with move.
...
ENGINE_ALL_COLOR_TO_A: ; 1F:0DFE, 0x03EDFE
    PHA ; Save passed.
    JSR ENGINE_PALETTE_COMMIT_TO_SCRIPT ; Move colors.
...
LOOP_PALETTE_WRITE: ; 1F:0E05, 0x03EE05
    STA ENGINE_COMMIT_PALETTE[32],X ; Store to palette commit.
    DEX ; X--
    BPL LOOP_PALETTE_WRITE ; Positive, goto.
    JMP CREATE_PALETTE_UPLOAD_PACKET_0x1_WAIT ; Upload.
...
LOOP_ALL_INDEXES: ; 1F:0E84, 0x03EE84
    ...
    RTS ; Leave.
ENGINE_PALETTE_COMMIT_TO_SCRIPT: ; 1F:0E8E, 0x03EE8E
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    LDX #$1F ; Index.
...
ENGINE_SETTLE_AND_PALETTE_FROM_PTR: ; 1F:0E9D, 0x03EE9D
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Init FPTR.
    STX LIB_BCD/EXTRA_FILE_BCD_B
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    LDY #$1F ; Index.
COUNT_POSITIVE: ; 1F:0EA6, 0x03EEA6
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; From index.
...
ENGINE_HELPER_SETTLE_CLEAR_LATCH_SET_SCROLL_TODO_MORE: ; 1F:0ECC, 0x03EECC
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle updates.
    LDA #$00
    STA NMI_FP_BATTLE_UNK[2] ; Clear ptr.
    STA NMI_FP_BATTLE_UNK+1
    ...
    STY ENGINE_SCROLL_Y
    JMP ENGINE_SET_NMI_FLAG_UPDATE_TODO_WAIT ; Settle 1 frame.
SCRIPT_INVERT_X_SCROLL_SETTLE: ; 1F:0EE4, 0x03EEE4
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle updates.
    LDA #$04
    EOR ENGINE_SCROLL_X ; Invert scroll bit.
    STA ENGINE_SCROLL_X ; Store back.
    JMP ENGINE_SET_NMI_FLAG_UPDATE_TODO_WAIT ; Set wait, abuse RTS.
SCRIPT_SCROLL_INVERT_RTN?: ; 1F:0EF0, 0x03EEF0
    ...
    ASL LIB_BCD/EXTRA_FILE_BCD_A ; Shift it.
    BCC RTS ; CC, leave.
    JSR RANDOMIZE_GROUP_A ; Adds ??
    AND #$C0 ; Keep 1100.0000
    BNE RTS ; Nonzero, leave.
    JSR ENGINE_PALETTE_COMMIT_TO_SCRIPT ; Do.
...
ENGINE_MENU_INIT_MASTER_FULL: ; 1F:0F34, 0x03EF34
    ...
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Move submenu ptr.
    STA FPTR_MENU_SECONDARY/SUBMENU[2] ; Ptr L.
    INY ; Stream++
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Move rows.
    STA FPTR_MENU_SECONDARY/SUBMENU+1 ; Ptr H.
ENGINE_MENU_INIT_MASTER_PARTIAL: ; 1F:0F3F, 0x03EF3F
    LDY #$06 ; Stream index.
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Move coord base.
...
MENU_FILE_MAKE_ROW_OFFSET?: ; 1F:0F4B, 0x03EF4B
    LDY #$00 ; Stream reset.
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Load from stream.
    STA MENU_COLUMN_INDEX ; Set ??
    TAX ; Val to X for multiply.
    LDY #$01 ; Stream set.
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Load from stream. Max selection in line?
    JSR ENGINE_MULTIPLY_X/A_TO_UINT ; Multiply with.
...
SUBMENU_FIND_VALID_OPTION: ; 1F:0F5F, 0x03EF5F
    LDA [FPTR_MENU_SECONDARY/SUBMENU[2]],Y ; Load from submenu.
...
SETTLE_AND_MENU_COORD_SPRITE_HELPER?: ; 1F:0F7C, 0x03EF7C
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle updates.
    LDY #$18 ; Seed wait.
    STY SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1
    LDA #$00
    STA SPRITE_PAGE+2 ; Clear attrs.
TABLE_REPORT_INVALID: ; 1F:0F88, 0x03EF88
    LDY #$05 ; Stream index.
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Move to cursor tile.
    STA SPRITE_PAGE+1
    LDY #$02 ; Stream index.
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Load ?? from main.
    LDX MENU_COLUMN_INDEX ; Load which.
    JSR ENGINE_MULTIPLY_X/A_TO_UINT ; Multiply file with selected.
    CLC ; Prep add.
    ...
    JSR ENGINE_MULTIPLY_X/A_TO_UINT ; Multiply.
    CLC ; Prep add.
...
Y_LOOPS_CHECK: ; 1F:0FBC, 0x03EFBC
    ...
    LDY #$05
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Load cursor value to invert to 0x00. Smart.
...
BUTTONS_PREVIOUSLY_PRESSED: ; 1F:0FDD, 0x03EFDD
    LDY #$06
    STY SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Reseed check wait.
CONTROLS_PRESSED: ; 1F:0FE1, 0x03EFE1
    ...
    LDY #$04 ; File index.
    AND #$F0 ; Keep upper.
    AND [FPTR_MENU_PRIMARY[2]],Y ; Mask with file.
...
LOOP_SELECTY?: ; 1F:1010, 0x03F010
    ...
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Load from FPTR.
    LDX LIB_BCD/EXTRA_FILE_BCD_A ; X from, valid row?
    JSR ENGINE_MULTIPLY_X/A_TO_UINT ; Multiply.
    CLC ; Prep add.
...
MENU_SELECTION_INVALID: ; 1F:1055, 0x03F055
    ...
    AND [FPTR_MENU_PRIMARY[2]],Y ; Mask with.
...
ENGINE_CURSOR_TO_BG_UPDATE_INTO_MENU?: ; 1F:10B0, 0x03F0B0
    PHA ; Save A.
    LDY #$02 ; Stream index.
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Load X pos related?
    LDX MENU_COLUMN_INDEX ; Load index.
    JSR ENGINE_MULTIPLY_X/A_TO_UINT ; Do.
    CLC ; Prep add.
    ADC GFX_COORD_HORIZONTAL_OFFSET ; Add with.
    STA GFX_COORD_HORIZONTAL_OFFSET ; Store to.
    LDY #$03 ; Stream index.
    LDA [FPTR_MENU_PRIMARY[2]],Y ; Load row height.
...
ACTION_MENU_ROW_MOD: ; 1F:10EA, 0x03F0EA
    ...
    .db FF ; -1 L
    .db 00 ; +0
LIB_MATH_16BIT_MULTIPLY?: ; 1F:10F1, 0x03F0F1
    LDA #$00 ; Init clear.
    LDX #$10 ; Bits to check count.
VAL_NONZERO: ; 1F:10F5, 0x03F0F5
    ROR LIB_BCD/EXTRA_FILE_BCD_B ; Rotate bits.
    ROR LIB_BCD/EXTRA_FILE_BCD_A
    BCC ROTATE_CC
    CLC ; Prep add.
    ADC SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Add val.
ROTATE_CC: ; 1F:10FE, 0x03F0FE
    ...
    ROR LIB_BCD/EXTRA_FILE_BCD_A
    RTS ; Return.
LIB_MATH_24BIT_MULTIPLY?: ; 1F:1109, 0x03F109
    LDA #$00 ; Clear init.
    LDX #$18 ; Bits to check count.
VAL_NONZERO: ; 1F:110D, 0x03F10D
    ROR LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Rotate bits.
    ROR LIB_BCD/EXTRA_FILE_BCD_B
    ROR LIB_BCD/EXTRA_FILE_BCD_A
...
ROTATE_CLEAR: ; 1F:1118, 0x03F118
    ...
    ROR LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Rotate all bits.
    ROR LIB_BCD/EXTRA_FILE_BCD_B
    ROR LIB_BCD/EXTRA_FILE_BCD_A
    RTS ; Leave.
ENGINE_MULTIPLY_X/A_TO_UINT: ; 1F:1125, 0x03F125
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Store bits.
    STX SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Store mod? Multiply?
    LDA #$00 ; Seed clear.
    LDX #$08 ; Loop count.
LOOPS_NONZERO: ; 1F:112D, 0x03F12D
    ROR LIB_BCD/EXTRA_FILE_BCD_A ; OUT: Unique bit. IN: Bit rotate out from A.
    BCC ROTATE_CLEAR ; Clear, goto.
    CLC ; Prep add.
    ADC SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Add with, stride per bit set.
ROTATE_CLEAR: ; 1F:1134, 0x03F134
    ROR A ; Rotate A. TODO: Why/result.
    DEX ; Loops--
    BNE LOOPS_NONZERO ; != 0, goto.
    TAX ; Val to X, high byte.
    LDA LIB_BCD/EXTRA_FILE_BCD_A ; Load low byte.
    ROR A ; Rotate final into.
    RTS ; Leave.
ENGINE_MATH_24BIT_DIVIDE?: ; 1F:113D, 0x03F13D
...
ENGINE_HOLD_FOREVER_IF_ZERO: ; 1F:113F, 0x03F13F
    ...
    LDX #$18 ; Bits count.
    ROL LIB_BCD/EXTRA_FILE_BCD_A ; Rotate bits to seed it all.
    ROL LIB_BCD/EXTRA_FILE_BCD_B
    ROL LIB_BCD2/EXTRA_FILE_STREAM_INDEX
PROCESS_VALUE_LOOP: ; 1F:114B, 0x03F14B
    ROL A ; Into A.
    BCS SUB_AND_ROTATE_RESULT ; CS, goto.
    CMP SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; If _ val
    BCC VAL_LT_VAR ; <, goto. Is okay.
SUB_AND_ROTATE_RESULT: ; 1F:1152, 0x03F152
    SBC SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Sub with value. Either for overflow or set.
    SEC ; Bring in 0x1 on rotate.
VAL_LT_VAR: ; 1F:1155, 0x03F155
    ROL LIB_BCD/EXTRA_FILE_BCD_A ; Rotate into.
    ROL LIB_BCD/EXTRA_FILE_BCD_B
    ROL LIB_BCD2/EXTRA_FILE_STREAM_INDEX
    DEX ; Todo--
    BNE PROCESS_VALUE_LOOP ; !=, goto.
...
LOOP_FIGURE_SINGLE_DIGIT: ; 1F:1163, 0x03F163
    ...
    ROL LIB_BCD/EXTRA_FILE_BCD_A ; Rotate to begin.
    ROL LIB_BCD/EXTRA_FILE_BCD_B
    ROL LIB_BCD2/EXTRA_FILE_STREAM_INDEX
...
VAL_IN_DECIMAL_RANGE: ; 1F:1175, 0x03F175
    ROL LIB_BCD/EXTRA_FILE_BCD_A ; Rotate again. CC in if no overflow. CS otherwise.
    ROL LIB_BCD/EXTRA_FILE_BCD_B
    ROL LIB_BCD2/EXTRA_FILE_STREAM_INDEX
    DEX ; Loops--
    ...
    LDA LIB_BCD/EXTRA_FILE_BCD_A ; Combine all.
    ...
    STY LIB_BCD/EXTRA_FILE_D ; Store initial digit index here.
...
CLEAR_MODSTUFF_AND_GOTO: ; 1F:11A4, 0x03F1A4
    LDY #$00
    STY LIB_BCD/EXTRA_FILE_BCD_A
    STY LIB_BCD/EXTRA_FILE_BCD_B
    STY LIB_BCD2/EXTRA_FILE_STREAM_INDEX
    BEQ SYNC_CLEARED
...
LIB_BATTLE_ENDED?: ; 1F:1202, 0x03F202
    ...
    LDX #$2C ; Val ??
    LDY #$09 ; Stream index ??
    LDA [BATTLE_PARTY_FPTR_DATA_TODO[2]],Y ; Load from created in sub above.
    AND #$F0 ; Keep upper.
    CMP #$50 ; If _ #$50
...
ENGINE_SET_MAPPER_R6_TO_0x16: ; 1F:1239, 0x03F239
    ...
    PLA
    RTS ; Leave.
ENGINE_SCRIPT_HELP_MAPPER_R6_TO_0x00: ; 1F:1247, 0x03F247
    PHA ; Save A,X.
    TXA
    PHA
    LDA #$00
...
RTS: ; 1F:126A, 0x03F26A
    RTS
SCRIPT_BATTLE_HELPER_BG_COLORS?: ; 1F:126B, 0x03F26B
    INX ; Index++
LOOP_SCRIPTY_BG: ; 1F:126C, 0x03F26C
    ...
    JMP PALETTE_MOD_BG_COLOR_TO_0x00 ; Do to black.
...
KEEP_ITERATING: ; 1F:127E, 0x03F27E
    ...
    JSR ENGINE_SET_NMI_FLAG_UPDATE_TODO_WAIT ; Wait settle.
    JSR ENGINE_SET_NMI_FLAG_UPDATE_TODO_WAIT ; 2x
    PLA ; Pull iteration.
    TAX ; To X.
    DEX ; --
...
A/B_NOT_PRESSED: ; 1F:12A2, 0x03F2A2
    ...
    RTS ; Leave.
SWITCH_TABLE_PAST_JSR_WITH_PTR[0]_AS_RET_CATCH: ; 1F:12AE, 0x03F2AE
    ASL A ; << 1, *2.
    TAY ; Val to Y.
    ...
    PLA
    STA LIB_BCD/EXTRA_FILE_BCD_B
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Load from JSR file.
    STA LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Store to call.
    INY ; Stream++
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Load from file.
    STA LIB_BCD/EXTRA_FILE_D ; Call H.
    LDY #$01 ; Reseed into first ptr.
    SEC ; Prep sub for RTS fix to Addr-0x1
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Load from header, RTS PTR L.
    SBC #$01 ; -= 0x1
    TAX ; Save to X.
    INY ; Stream++
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Load from file, RTS PTR H.
    SBC #$00 ; Carry sub.
    PHA ; Save back to stack.
    TXA ; Save lower.
    PHA
    JMP [LIB_BCD2/EXTRA_FILE_STREAM_INDEX] ; Run main routine with catch.
SWITCH_TABLE_PAST_JSR_HELPER: ; 1F:12D5, 0x03F2D5
    ASL A ; << 1, *2. To word offset.
    TAY ; To Y.
    INY ; Offset stream data.
    PLA ; Pull address from stack.
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Address to fptr.
    PLA
    STA LIB_BCD/EXTRA_FILE_BCD_B
    SEC ; Prep sub.
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Load from file.
    ...
    TAX ; To X.
    INY ; Stream++
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Load PTR H.
    SBC #$00 ; Carry sub.
    PHA ; Save addr created.
    TXA
    PHA
    RTS ; Run the switch option.
SCRIPT_HARD_SWITCH_HUGE_FILES_TODO: ; 1F:12ED, 0x03F2ED
    PHA ; Save A,X,Y.
    TXA
    PHA
    TYA
    PHA
    LDA LIB_BCD/EXTRA_FILE_D ; Save.
    PHA
    LDA LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Save.
    PHA
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Save.
    PHA
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Save.
    PHA
...
LOOP_ROTATE: ; 1F:130B, 0x03F30B
    ...
    ROL LIB_BCD/EXTRA_FILE_BCD_B ; Roll into.
    DEX ; Count--
    BNE LOOP_ROTATE ; != 0, loop.
    STX LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Clear.
    TXA ; Clear A.
    PHA ; 0x00 to stack.
    LDA LIB_BCD/EXTRA_FILE_BCD_B ; Save shifty.
    PHA
    LDA LIB_BCD/EXTRA_FILE_BCD_A
    PHA
    LDA #$64
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Seed PTR L.
    JSR ENGINE_MATH_24BIT_DIVIDE? ; Do numbers.
    ...
    JSR LIB_MATH_16BIT_MULTIPLY? ; Do 
    PLP ; Pull status.
...
LOOP_SHIFT: ; 1F:1357, 0x03F357
    LSR LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Shift.
    ROR LIB_BCD/EXTRA_FILE_BCD_B ; Rotate into.
    ROR LIB_BCD/EXTRA_FILE_BCD_A ; 2x
    DEX ; Count--
...
ROM_TABLE_RANDOM_UNK: ; 1F:137D, 0x03F37D
    ...
    .db 17
    .db 18
ENGINE_LIB_STASH_EXTRA_FILE_AND_SWITCH_UNK: ; 1F:13FD, 0x03F3FD
    TAX ; A to X.
    LDA LIB_BCD/EXTRA_FILE_BCD_B ; Move to stack.
    PHA
    LDA LIB_BCD/EXTRA_FILE_BCD_A
    PHA
    STX LIB_BCD/EXTRA_FILE_BCD_A ; X to.
    LDA #$00
    STA LIB_BCD/EXTRA_FILE_BCD_B ; Clear ??
    JSR SCRIPT_HARD_SWITCH_HUGE_FILES_TODO ; Do ??
    LDA LIB_BCD/EXTRA_FILE_BCD_B ; Load ??
    BEQ VAL_EQ_0x00 ; == 0, goto.
    LDA #$FF
...
VAL_EQ_0x00: ; 1F:1415, 0x03F415
    ...
    PLA
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Restore old.
    PLA
    STA LIB_BCD/EXTRA_FILE_BCD_B
    TXA ; X to A.
    RTS ; Leave.
LIB_PLAY_BATTLE_SFX_A_PASSED_WITH_SCRIPT_DOING_TODO: ; 1F:141F, 0x03F41F
    PHA ; Save A, SFX ID playing.
    ASL A ; << 2, *4. Slot index.
    ASL A
    BEQ EXIT_RETURN_REQUESTED ; == 0, goto.
    TAX ; Slot index to X.
    LDA BATTLE_SFX_ID_COMMITED,X ; Move SFX for slot 0x00.
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Store, SFX ID to put.
    LDA BATTLE_SFX_WAIT_TIME,X ; Move time for SFX happening.
    STA ALT_COUNT_UNK ; 2x
    LDA BATTLE_SFX_SLOT_WRITTEN,X ; Load slot going to.
    CMP #$00 ; If _ #$00
    BNE SWITCH_A_FAIL ; != 0, goto.
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Move to SFX[0]
    STA SOUND_EFFECT_REQUEST_ARRAY[5]
    JMP SFX_PLAYED ; Past.
SWITCH_A_FAIL: ; 1F:143E, 0x03F43E
    CMP #$01 ; If _ #$01
    BNE SWITCH_B_FAIL ; != 0, goto.
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Move to SFX[1]
    STA SOUND_EFFECT_REQUEST_ARRAY+1
    JMP SFX_PLAYED ; Past.
SWITCH_B_FAIL: ; 1F:144A, 0x03F44A
    CMP #$02 ; If _ #$02
    BNE SWITCH_DEFAULT ; != 0, goto.
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Move to SFX[3]
    STA SOUND_EFFECT_REQUEST_ARRAY+3
    JMP SFX_PLAYED ; Past.
SWITCH_DEFAULT: ; 1F:1456, 0x03F456
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Move to SFX[4]
    STA SOUND_EFFECT_REQUEST_ARRAY+4
SFX_PLAYED: ; 1F:145B, 0x03F45B
    LDX ALT_COUNT_UNK ; Load count.
    JSR ENGINE_WAIT_X_SETTLES ; Wait that long.
    JSR ENGINE_SET_MAPPER_R6_TO_0x16 ; Set mapper.
EXIT_RETURN_REQUESTED: ; 1F:1463, 0x03F463
    PLA ; Pull A.
    RTS ; Leave.
ENGINE_UNK_PLAY_CURSOR_A_LOT_TODO_UNK: ; 1F:1465, 0x03F465
    LDX #$0F ; Index count.
LOOP_WRITE_SFX_ALOT: ; 1F:1467, 0x03F467
    TXA ; Count to stack.
    PHA
    LDA #$05
    STA SOUND_EFFECT_REQUEST_ARRAY+1 ; Set SFX.
    LDX #$02 ; Wait.
    JSR ENGINE_WAIT_X_SETTLES
    PLA ; Pull count.
    TAX
    DEX ; Count--
    BNE LOOP_WRITE_SFX_ALOT ; != 0, loop.
    RTS ; Leave.
LIB_VAL_TO_DECIMAL_AND_FILE?: ; 1F:1479, 0x03F479
    LDA SCRIPT_OVERWORLD_BATTLE_ENCOUNTER? ; Val ??
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Move.
    LDA #$00
    STA LIB_BCD/EXTRA_FILE_BCD_B ; Clear bits upper.
    LDA #$0A
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Mod val.
    JSR LIB_MATH_16BIT_MULTIPLY? ; Do ??
    CLC
    LDA #$98 ; Load ??
    ADC LIB_BCD/EXTRA_FILE_BCD_A ; Add with.
    STA BATTLE_PARTY_FPTR_DATA_TODO[2] ; Store result.
    LDA #$8F ; Load.
    ADC LIB_BCD/EXTRA_FILE_BCD_B ; Add ??
...
ENGINE_QUEUE_UPDATE_PALETTE_PACKET: ; 1F:14A3, 0x03F4A3
    ...
    STA NMI_FLAG_EXECUTE_UPDATE_BUF_AND_MORE_TODO ; Set flag.
    RTS ; Leave.
PALETTE_MOD_BG_COLOR_TO_0x00: ; 1F:14B6, 0x03F4B6
    LDA #$0F ; Val ??
PALETTE_MOD_BG_COLOR_TO_A: ; 1F:14B8, 0x03F4B8
    PHA ; Save value.
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    PLA ; Pull val.
    LDY #$1C ; Stream index BG color init seed.
INDEX_POSITIVE: ; 1F:14BF, 0x03F4BF
    ...
    JSR ENGINE_QUEUE_UPDATE_PALETTE_PACKET ; Queue palette update.
    JMP ENGINE_SET_NMI_FLAG_UPDATE_TODO_WAIT ; Wait.
LIB_BANK_0_FILES_COORDS_BATTLE_MENU?_UNK: ; 1F:14CE, 0x03F4CE
    ASL A ; << 1, *1. ID to word size. NOTE: R6 = 0x16 coming out.
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Store to.
    TXA ; Save X and Y.
    PHA
    TYA
    ...
    LDY LIB_BCD/EXTRA_FILE_BCD_A ; Load as index.
...
USE_XCOORD_ASIS: ; 1F:14F4, 0x03F4F4
    STX LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; X to. Coord?
    INY ; Stream++
...
USE_YCOORD_ASIS: ; 1F:1500, 0x03F500
    STX LIB_BCD/EXTRA_FILE_D ; Store val.
CONSUME_FILE_TODO_MENU?: ; 1F:1502, 0x03F502
    INY ; Stream++
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Load from stream.
    LDX #$00 ; Val ??
    CMP #$FC ; If _ #$FC
    BEQ INDEX_SEEDED_STORE ; ==, goto.
    LDX #$01 ; Val.
    CMP #$FD ; If _ #$FD
    BEQ INDEX_SEEDED_STORE ; ==, goto.
    LDX #$02 ; Val.
    ...
    JMP CONSUME_FILE_TODO_MENU? ; Goto, loop.
INDEX_SEEDED_STORE: ; 1F:1521, 0x03F521
    STX ALT_COUNT_UNK ; Store X seeded.
    JMP CONSUME_FILE_TODO_MENU? ; Loop, goto.
...
LIB_MOVE_PTR_LOL: ; 1F:152E, 0x03F52E
    ...
    INY ; Stream++
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Move pointer ??
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2]
    INY ; Stream++
...
X_NONZERO: ; 1F:1539, 0x03F539
    ...
    LDA #$00
    STA R_**:$0070 ; Clear ??
    LDA LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Move to HPos coord.
    STA GFX_COORD_HORIZONTAL_OFFSET
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Move packet creation ptr.
...
LIB_SAVE_STATE_FULL?: ; 1F:1562, 0x03F562
    LDA LIB_BCD/EXTRA_FILE_BCD_B ; Stack this stuff.
    PHA
    LDA LIB_BCD/EXTRA_FILE_BCD_A
    PHA
...
VAL_EQ_0x00: ; 1F:158D, 0x03F58D
    CLC ; Prep add.
    LDA LIB_BCD/EXTRA_FILE_D ; Load val.
    ADC ALT_STUFF_COUNT? ; Mod val.
    STA GFX_COORD_VERTICAL_OFFSET ; Store to.
    PHA ; Save it, too.
    JSR RTN_SETTLE_UPDATE_TODO ; Do.
    PLA ; Pull saved.
    STA LIB_BCD/EXTRA_FILE_D ; Restore it.
    JMP LIB_STATE_RESTORE ; Restore.
...
LIB_STATE_RESTORE: ; 1F:15AC, 0x03F5AC
    ...
    STA LIB_BCD/EXTRA_FILE_BCD_B
    RTS ; Leave.
SAVE_FLAG_SET_SUBMENU_AND_MENU_SINGLE_UNK_TODO: ; 1F:15C2, 0x03F5C2
    LDA FLAG_SPRITE_OFF_SCREEN_UNK ; Load ??
    PHA ; Save it.
    JSR CLEAR_FLAG_OFFSCREEN ; Do.
    LDA #$DF
    STA FPTR_MENU_SECONDARY/SUBMENU[2] ; Set up PTR, 1F:15DF data table.
    LDA #$F5
    STA FPTR_MENU_SECONDARY/SUBMENU+1
    LDA #$DF
    STA FPTR_MENU_PRIMARY[2] ; Set up ptr, 1F:15DF data table.
    LDA #$F5
    ...
    RTS ; Leave.
    .db 01 ; Main and secondary menu ??
    .db 01
    .db 00
    .db 00
    .db C0
    .db 5D
LIB_SCRIPT_HELPER_CREATE_DISPLAY_PACKET_HELPER: ; 1F:15E5, 0x03F5E5
    PHA ; Save A,X,Y.
    TXA
    PHA
    TYA
    PHA
    JSR ENGINE_PACKET_CREATE_BATTLE_UNK_NO_MASKING ; Do ??
    JSR SCRIPT_ENGINE_HELPER_SETTLE_AND_STUFF_TODO ; Do ??
    PLA ; Restore A,X,Y.
    TAY
    PLA
    TAX
...
LIB_PTR_CREATE_UNK: ; 1F:15F7, 0x03F5F7
    STA LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; A to.
    LDA #$00 ; Init clear.
    ASL LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; ASL/ROT 3x
    ROL A ; Into A.
    ASL LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; 2x
    ROL A
    ASL LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; 3x
    ROL A
    STA LIB_BCD/EXTRA_FILE_D ; A result to.
    CLC
    LDA LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Base created PTR at $9E00, R?
    ADC #$00 ; += 0x00
    STA LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Store result.
    LDA LIB_BCD/EXTRA_FILE_D ; Load.
    ADC #$9E ; += 0x9E
    STA LIB_BCD/EXTRA_FILE_D ; Store result.
    RTS ; Leave.
SCRIPT_ENGINE_HELPER_SETTLE_AND_STUFF_TODO: ; 1F:1614, 0x03F614
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    LDY #$E8
...
VAL_NE_0x60: ; 1F:1630, 0x03F630
    ...
    LDA SCRIPT_PARTY_ATTRIBUTES+17,Y ; Load really indexed.
    AND #$06 ; Keep 0000.0110
    EOR #$06 ; Invert them.
    BEQ DATA_INDEXED_0x00 ; == 0, goto.
    LDX #$02 ; Seed main.
    LDA SCRIPT_PARTY_ATTRIBUTES+1,Y ; Load from pair.
    AND #$80 ; Keep neg.
...
ROUTINE_OBJ_HANDLE_UNK_A: ; 1F:1673, 0x03F673
    TYA ; Save Y
    PHA
    LDA PARTY_ATTR_PTR[2],Y ; Move from Y index to MISC.
    STA LIB_BCD/EXTRA_FILE_BCD_A
    LDA PARTY_ATTR_PTR+1,Y
    STA LIB_BCD/EXTRA_FILE_BCD_B
    LDA SCRIPT_PARTY_ATTRIBUTES+3,Y ; Move to mod ptr?
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2]
    ...
    STA LIB_BCD/EXTRA_FILE_D ; Stopre to ??
    LSR LIB_BCD/EXTRA_FILE_D ; Shift it into.
    ROR LIB_BCD2/EXTRA_FILE_STREAM_INDEX
    LSR LIB_BCD/EXTRA_FILE_D ; 2x
    ROR LIB_BCD2/EXTRA_FILE_STREAM_INDEX
    PLA ; Restore Y.
    TAY
    SEC ; Prep sub.
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Load ??
    SBC LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Sub with created.
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Load other.
    SBC LIB_BCD/EXTRA_FILE_D ; Sub with. other.
    RTS ; Leave.
SETTLE_AND_TABLE_IN_A: ; 1F:16AA, 0x03F6AA
    PHA ; Save A.
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    PLA ; Restore.
    JSR SWITCH_TABLE_PAST_JSR_WITH_PTR[0]_AS_RET_CATCH ; Do routine passed in A.
    LOW(IN-CODE_PTR_TABLE_A_FLAG_UPDATE_BUF) ; Set flag for buf upload.
    HIGH(IN-CODE_PTR_TABLE_A_FLAG_UPDATE_BUF)
...
IN-CODE_PTR_TABLE_A_FLAG_UPDATE_BUF: ; 1F:16BA, 0x03F6BA
    LDA #$01
    STA NMI_FLAG_EXECUTE_UPDATE_BUF_AND_MORE_TODO ; Set flag ??
    RTS
...
IN-CODE_PTR_TABLE_C: ; 1F:16C8, 0x03F6C8
    ...
    LDA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],X ; Obj? Save.
    PHA
...
OBJECT_STORE_UNK: ; 1F:16F9, 0x03F6F9
    STX LIB_BCD/EXTRA_FILE_BCD_A ; X and Y to. Will go to obj.
    STY LIB_BCD/EXTRA_FILE_BCD_B
    LDX RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY[8] ; Load index.
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],X ; To A passed.
    LDA #$08 ; Obj ??
    ...
    STA OBJ?_BYTE_0x5_BYTE,X
    LDA LIB_BCD/EXTRA_FILE_BCD_A ; Move passed PTR to.
    STA OBJ?_PTR?[2],X
    LDA LIB_BCD/EXTRA_FILE_BCD_B
    STA OBJ?_PTR?+1,X
    RTS ; Leave.
OBJ_SET_AND_SETTLE_RETURN: ; 1F:1724, 0x03F724
    LDX RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY[8] ; Load index.
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],X ; Store passed as status.
    LDA #$01
    STA NMI_FLAG_EXECUTE_UPDATE_BUF_AND_MORE_TODO ; Set flag.
    LDX #$08
...
ENGINE_PALETTE_SIZE_UPDATE_FPTR_XY: ; 1F:1732, 0x03F732
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    STX LIB_BCD/EXTRA_FILE_BCD_A ; X and Y to FPTR.
    STY LIB_BCD/EXTRA_FILE_BCD_B
    LDY #$1F ; Index.
...
SPRITE_OFF_SCREEN_THINGY: ; 1F:1772, 0x03F772
    ...
    TXA ; Clear A.
    STA SCRIPT_BATTLE_UNK_0x59 ; Clear ??
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+224 ; Clear ??
    LDA ENGINE_NMI_HELPER_SLOT_OFFSCREEN_ALWAYS_NMI_CURSOR_DELETE ; Load ??
    ASL A ; << 2, *4.
    ASL A
    TAY ; To Y index.
    LDA #$F0 ; Set Xpos.
...
VECTOR_NMI: ; 1F:179F, 0x03F79F
    ...
    BIT ENGINE_NMI_CONFIG_FLAGS_TODO_BETTER ; Test flag.
...
NO_LATCH: ; 1F:1827, 0x03F827
    ...
    LDA #$80
    STA ENGINE_NMI_CONFIG_FLAGS_TODO_BETTER ; Set ??
...
VAL_EQ_0x00: ; 1F:18B7, 0x03F8B7
    LDA #$00
    STA ENGINE_NMI_CONFIG_FLAGS_TODO_BETTER ; Clear flags.
    PLA ; Leave NMI.
    TAY
    PLA
    TAX
...
COUNT_POSITIVE: ; 1F:1A96, 0x03FA96
    CLC ; Prep add.
    LDA [NMI_FP_BATTLE_UNK[2]],Y ; Load from stream.
    ...
    CLC ; Prep add.
    TYA ; Stream to A.
    ADC NMI_FP_BATTLE_UNK[2] ; Add with.
    STA NMI_FP_BATTLE_UNK[2] ; Store result.
    LDA #$00 ; Carry seed.
    ADC NMI_FP_BATTLE_UNK+1 ; Add carry.
    STA NMI_FP_BATTLE_UNK+1 ; Store result.
    JMP ALT_ENTRY ; Goto.
LOOP_ADD_PAIR_UNK: ; 1F:1AB8, 0x03FAB8
    CLC ; Prep add.
    LDA NMI_FP_BATTLE_UNK[2] ; Load ??
    ADC R_**:$00CE ; Add with.
    STA R_**:$00CE ; Store result.
    CLC ; Prep add.
    LDA NMI_FP_BATTLE_UNK+1 ; Load ??
...
ITERATION_START: ; 1F:1B0B, 0x03FB0B
    ...
    LDA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],Y ; Load from.
    AND #$3F ; Keep lower.
...
OBJ_UNK_INVERTED: ; 1F:1BA0, 0x03FBA0
    LDA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],Y ; Load obj.
    EOR #$80 ; Invert ??
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],Y ; Store inverted.
...
ADD_CC: ; 1F:1BCB, 0x03FBCB
    LDA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],Y ; Load from object.
    AND #$80 ; Keep top bit.
...
CLEAR_PAGE: ; 1F:1CF1, 0x03FCF1
    ...
    STA ENGINE_PPU_CTRL_COPY
ENGINE_HELPER_PPU_0x1000_QUAD_PAGES/RENDERING/V_MIRROR: ; 1F:1D00, 0x03FD00
    LDA #$80
...
JUST_WAIT: ; 1F:1D30, 0x03FD30
    JMP ENGINE_SET_NMI_FLAG_UPDATE_TODO_WAIT ; Wait, abuse RTS.
...
ENGINE_DELAY_X_FRAMES: ; 1F:1D3A, 0x03FD3A
    ...
    RTS ; Leave.
ENGINE_SET_NMI_FLAG_UPDATE_TODO_WAIT: ; 1F:1D41, 0x03FD41
    LDA #$01
    STA ENGINE_NMI_CONFIG_FLAGS_TODO_BETTER ; Set flag.
WAIT_COMPLETION: ; 1F:1D45, 0x03FD45
    LDA ENGINE_NMI_CONFIG_FLAGS_TODO_BETTER ; Load.
...
WAIT_CONTROL_INPUT_LOOP: ; 1F:1D53, 0x03FD53
    LDA CONTROL_ACCUMULATED?[2] ; Load.
    BEQ WAIT_CONTROL_INPUT_LOOP ; == 0, wait longer for press.
    PHA ; Save buttons.
    LDA #$00
    STA CONTROL_ACCUMULATED?[2] ; Clear accumulated.
    PLA ; Restore buttons pressed.
    RTS ; Leave.
SETTLE_SPRITES_OFFSCREEN/CLEAR_OBJ_EXTRA_MEMORY: ; 1F:1D5E, 0x03FD5E
...
SLOT_NONZERO: ; 1F:1D66, 0x03FD66
    LDA #$00 ; Clear.
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],X ; Clear indexed.
