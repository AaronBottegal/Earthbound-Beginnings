...
MAP_RTN_A: ; 1F:01BE, 0x03E1BE
    ...
    SEC
    LDA **:$003C
    SBC SCRIPT_PAIR_UNK[2]
    STA SAVE_GAME_MOD_PAGE_PTR[2]
    LDA **:$003D
    SBC SCRIPT_PAIR_UNK+1
...
TARGET_ITERATION: ; 1F:0DE1, 0x03EDE1
    LDX #$1F ; Index ??
LOOP_POSITIVE: ; 1F:0DE3, 0x03EDE3
    SEC ; Prep sub.
...
VAL_GTE_0x10: ; 1F:0DED, 0x03EDED
    ...
    JSR FINALIZE_PACKET_PALETTE_WAIT_X ; Queue update, delay iteration is delay frames, nice.
    DEY ; Loops--
    CPY #$01 ; If _ #$01
...
LOOP_PALETTE_WRITE: ; 1F:0E05, 0x03EE05
    ...
    JMP FINALIZE_PACKET_PALETTE_WAIT_0x1 ; Goto, abuse RTS.
...
PALETTE_POSITIVE: ; 1F:0E15, 0x03EE15
    ...
    JMP FINALIZE_PACKET_PALETTE_WAIT_0x1 ; Goto.
...
TARGET_REACHED: ; 1F:0E71, 0x03EE71
    ...
    TAX
    JSR FINALIZE_PACKET_PALETTE_WAIT_X ; Wait.
    DEY ; Iteration--
    CPY #$01 ; If _ #$01
...
COUNT_POSITIVE: ; 1F:0EA6, 0x03EEA6
    ...
    BMI FINALIZE_PACKET_PALETTE_WAIT_0x1 ; Always taken, delay 0x1.
ENGINE_PALETTE_UPLOAD_WITH_WAIT_HELPER: ; 1F:0EB0, 0x03EEB0
    JSR ENGINE_PALETTE_SCRIPT_TO_UPLOADED
FINALIZE_PACKET_PALETTE_WAIT_0x1: ; 1F:0EB3, 0x03EEB3
    LDX #$01 ; Seed 0x1 wait.
FINALIZE_PACKET_PALETTE_WAIT_X: ; 1F:0EB5, 0x03EEB5
...
ENGINE_SETTLE_EXTENDED_0x2000_SCREEN: ; 1F:0EC8, 0x03EEC8
    LDX #$00 ; Seed scroll.
    LDY #$00
    JSR ENGINE_SETTLE_UPDATES? ; Settle updates.
    LDA #$00
...
LOOP_X_TIMES: ; 1F:0F0C, 0x03EF0C
    ...
    JSR ENGINE_PALETTE_UPLOAD_WITH_WAIT_HELPER ; Colors.
...
INDEX_SKIP: ; 1F:0F2E, 0x03EF2E
    ...
    JMP FINALIZE_PACKET_PALETTE_WAIT_0x1 ; Upload palette.
MENU_SELECTION: ; 1F:0F34, 0x03EF34
    LDY #$08 ; Stream index.
    ...
    STA FPTR_UNK_84_MENU_SELECTION? ; Store result.
    LDY #$00 ; Stream set.
    STY CARRY_UP? ; Clear ??
LOOP_LT_TARGET: ; 1F:0F5F, 0x03EF5F
    ...
    INY ; Stream++
    CPY FPTR_UNK_84_MENU_SELECTION? ; If _ var
    BCC LOOP_LT_TARGET ; <, do more.
    STA FPTR_UNK_84_MENU_SELECTION? ; Clear ??
    STA **:$0083
    RTS ; Leave.
STREAM_NONZERO: ; 1F:0F6D, 0x03EF6D
    STY FPTR_UNK_84_MENU_SELECTION? ; Stream to.
    TYA ; Stream to A.
...
STREAM_POSITIVE: ; 1F:149B, 0x03F49B
    ...
    LDA #$04
    STA NMI_PPU_CMD_PACKETS_BUF[16] ; Queue palette update.
    LDA #$00
...
VAL_POSITIVE: ; 1F:173B, 0x03F73B
    LDA [ENGINE_PALETTE_FPTR/BITS/GEN_USE[2]],Y ; Load from stream.
    STA NMI_PPU_CMD_PACKETS_BUF[16],Y ; To.
    DEY ; Stream/index--
...
NMI_BUF_PROCESSOR_CHECK: ; 1F:17CA, 0x03F7CA
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Load from buf.
...
LOWER_DIRECTLY_TO_BUFFER: ; 1F:17DC, 0x03F7DC
    AND #$7F ; Keep bits.
    STA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Store to buf.
...
NMI_RESTORE_STATE: ; 1F:188A, 0x03F88A
    ...
    ORA ENGINE_MAPPER_CONFIG_STATUS ; Set other bits.
    ...
    LDA RAM_CODE_UNK[2] ; Load.
    BEQ VAL_EQ_0x00 ; == 0, goto.
    JSR RAM_CODE_UNK[2] ; Do sub.
...
RTN_0x02: ; 1F:18DB, 0x03F8DB
    ...
    TYA ; Index pos to A.
    SEC ; Prep with carry.
    ADC NMI_PPU_CMD_PACKETS_BUF[16],Y ; Mod with value.
    TAY ; Back to index.
    JMP NMI_BUF_PROCESSOR_CHECK ; Re-launch with new index.
RTN_0x03: ; 1F:18E5, 0x03F8E5
    INY ; Index++
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Load from buffer.
    TAY ; To new index.
...
PACKET_RELAUNCH: ; 1F:192A, 0x03F92A
    JSR NMI_PACKET_UNIQUE_DATA_UPLOADER ; Upload the packet.
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Load next packet.
    CMP #$06 ; Same type?
...
RTN_0x07: ; 1F:193C, 0x03F93C
    INY ; Get data.
    LDX NMI_PPU_CMD_PACKETS_BUF[16],Y ; Load count of single byte updates.
    INY
LOOP_UPDATES: ; 1F:1941, 0x03F941
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Move addr from buf.
    STA PPU_ADDR
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y
    STA PPU_ADDR
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Move data from buf.
    STA PPU_DATA
    INY
    DEX ; Count--
...
RTN_0x08: ; 1F:195C, 0x03F95C
    INY ; Buf++
    LDX NMI_PPU_CMD_PACKETS_BUF[16],Y ; Load count.
    INY ; Buf++
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Write addr.
    STA PPU_ADDR
    INY ; Buf++
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y
    STA PPU_ADDR
    INY ; Buf++
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Load data.
    INY ; Buf++
...
RTN_0x09: ; 1F:197C, 0x03F97C
    INY ; Buf++
    LDX NMI_PPU_CMD_PACKETS_BUF[16],Y ; Count.
    INY ; Buf++
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Move addr.
    STA PPU_ADDR
    INY ; Buf++
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y
    STA PPU_ADDR
...
READ_TO_UPDATE_BUF: ; 1F:1992, 0x03F992
    ...
    STA NMI_PPU_CMD_PACKETS_BUF[16],Y ; To buf.
    INY ; Buf++
    DEX ; Count--
...
MOVE_0x40_LOOP: ; 1F:19CD, 0x03F9CD
    ...
    PLA
    STA MAPPER_INDEX_LAST_WRITTEN
    ORA ENGINE_MAPPER_CONFIG_STATUS ; Set other CFG bits.
...
NMI_PACKET_UNIQUE_DATA_UPLOADER: ; 1F:19EF, 0x03F9EF
    INY ; Buf++
    LDX NMI_PPU_CMD_PACKETS_BUF[16],Y ; X from buf.
    STX UPDATE_PACKET_COUNT/GROUPS ; To, lower nibble count, upper nibble 0x8 groups.
    INY ; Buf++
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; PPU Addr
    STA PPU_ADDR
    INY ; Buf++
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y
    STA PPU_ADDR
    ...
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Move 1x Byte.
    STA PPU_DATA
    INY
TEST_0x2_FLAG_0x2_DBYTES: ; 1F:1A0F, 0x03FA0F
    ...
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Move 2x DBytes.
    STA PPU_DATA
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y
    STA PPU_DATA
...
TEST_0x4_FLAG_0x4_DBYTES: ; 1F:1A21, 0x03FA21
    ...
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Move 4x DBytes.
    STA PPU_DATA
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y
    STA PPU_DATA
...
X_LOOPS_NONZERO: ; 1F:1A45, 0x03FA45
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; Move 0x8 DBytes.
    STA PPU_DATA
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; 2x
    STA PPU_DATA
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; 3x
    STA PPU_DATA
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; 4x
    STA PPU_DATA
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; 5x
    STA PPU_DATA
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; 6x
    STA PPU_DATA
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; 7x
    STA PPU_DATA
    INY
    LDA NMI_PPU_CMD_PACKETS_BUF[16],Y ; 8x
    STA PPU_DATA
    INY
    DEX ; X--
    BNE X_LOOPS_NONZERO
...
CLEAR_PAGE: ; 1F:1CF1, 0x03FCF1
    ...
    STA ENGINE_PPU_CTRL_COPY
    LDA #$80
    STA ENGINE_MAPPER_CONFIG_STATUS ; Set four pages at PPU 0x1000.
...
ENGINE_SETTLE_UPDATES?: ; 1F:1D33, 0x03FD33
    LDA NMI_FLAG_E5_TODO ; Load.
    ORA NMI_FLAG_E0_TODO ; Or with other.
    BNE ENGINE_SETTLE_UPDATES? ; Nonzero, redo.
    RTS ; Leave.
...
SETTLE_SPRITES_OFFSCREEN/CLEAR_RAM: ; 1F:1D5E, 0x03FD5E
    JSR ENGINE_SETTLE_UPDATES? ; Wait for settle.
    SEC ; Shift in 0x1
    ROR NMI_FLAG_UNK ; Rotate, smol set.
...
SLOT_NONZERO: ; 1F:1D66, 0x03FD66
    ...
    ASL NMI_FLAG_UNK ; << 1, *2.
    RTS
ENGINE_CLEAR_SCREENS_0x2000-0x2800: ; 1F:1D80, 0x03FD80
    JSR ENGINE_SETTLE_UPDATES? ; Settle ??
    LDA #$08 ; Packet type.
    LDX #$80 ; Packet count.
    STA NMI_PPU_CMD_PACKETS_BUF[16] ; Store to buf.
...
LOOP_CLEARING_ADDR_ADJUST: ; 1F:1D9F, 0x03FD9F
    ...
    STA NMI_FLAG_E5_TODO ; Set flag ??
    JSR ENGINE_SETTLE_UPDATES? ; Settle engine.
    CLC ; Prep add.
...
CARRY_NO_OVERFLOW: ; 1F:1DD0, 0x03FDD0
    ...
    RTS ; Leave.
ENGINE_ENABLE_WRAM_WITH_WRITES: ; 1F:1DE7, 0x03FDE7
    LDA #$80
    STA MMC3_WRAM_CFG ; Set CFG, WRAM with writes.
    RTS
ENGINE_ENABLE_WRAM_NO_WRITES: ; 1F:1DED, 0x03FDED
    LDA #$C0
...
VECTOR_IRQ: ; 1F:1E13, 0x03FE13
    ...
    PLA ; Restore.
    ORA ENGINE_MAPPER_CONFIG_STATUS ; Set CFG.
...
ENGINE_SET_MAPPER_BANK_X_VAL_A: ; 1F:1FD0, 0x03FFD0
    ...
    TXA ; Bank slot to A.
    ORA ENGINE_MAPPER_CONFIG_STATUS ; Set config status bits.
