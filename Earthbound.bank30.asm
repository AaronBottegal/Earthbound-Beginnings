...
DELAY_LOOP_Y_A: ; 1E:0210, 0x03C210
    ...
    JMP EXIT_DELAY_SEED_0x4 ; Goto.
LIB_WRITE_LATCH_AND_RETURN_R2_GFX_INDEX: ; 1E:0218, 0x03C218
    STA MMC3_IRQ_LATCH ; A to latch.
    LDA #$02 ; Load R2.
    ORA ENGINE_MAPPER_CONFIG_STATUS_NO_BANK ; Set mapper config.
    TAX ; To X, R2 base return.
EXIT_DELAY_SEED_0x4: ; 1E:0220, 0x03C220
    LDY #$04 ; Seed delay.
EXIT_DELAY_Y_COUNT: ; 1E:0222, 0x03C222
    DEY ; Y--
    BNE EXIT_DELAY_Y_COUNT ; != 0, loop delay.
    RTS ; Leave.
...
LATCH_NO_FLAG_RTN: ; 1E:0274, 0x03C274
    ...
    LDX #$B2
    LDY #$A2
    JSR ENGINE_SCRIPT_LAUNCH_R7_A_PTR_XY_WITH_RESTORE ; Script launch 19:02B3 TODO
    LDA #$00
    STA NMI_LATCH_FLAG_UNK ; Clear ??
    LDA #$FF
...
LOOP_AND_RESETTLE_FOR_UPDATE: ; 1E:02B6, 0x03C2B6
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    LDX #$00
...
LIB_PRG_HELPER_R6_TO_0x00: ; 1E:0322, 0x03C322
    LDA #$00 ; Bank.
    LDX #$06 ; R6.
    JMP ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Set it, abuse RTS.
    .db 20
    .db 01
    .db 17
    .db 21
...
PTR_SEEDED_UNK_K: ; 1E:03A0, 0x03C3A0
    LDA #$6A
    LDX #$92 ; Ptr seed 13:126B routine.
STORE_PACKET_PTR_AND_LATCH_QUEUE_UPDATE_CREATE?: ; 1E:03A4, 0x03C3A4
    STA FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2] ; Store PTR.
    STX FPTR_PACKET_CREATION/PTR_H_FILE_IDK+1
    LDA NMI_LATCH_FLAG_UNK ; Load.
    BEQ EXIT_FLAG_CLEAR ; == 0, goto.
    JMP LATCH_FLAG_RTN ; Do ??
EXIT_FLAG_CLEAR: ; 1E:03AF, 0x03C3AF
    JMP LATCH_NO_FLAG_RTN ; Goto.
PTR_SEEDED_UNK_A: ; 1E:03B2, 0x03C3B2
    LDA #$9B ; Seed ??:129B data for packet update.
    LDX #$92
    JMP STORE_PACKET_PTR_AND_LATCH_QUEUE_UPDATE_CREATE?
...
PTR_SEEDED_UNK_C: ; 1E:03C0, 0x03C3C0
    ...
    LDX #$93
    JMP STORE_PACKET_PTR_AND_LATCH_QUEUE_UPDATE_CREATE?
PTR_SEEDED_UNK_D: ; 1E:03C7, 0x03C3C7
    LDA #$17
    LDX #$93
    JMP STORE_PACKET_PTR_AND_LATCH_QUEUE_UPDATE_CREATE?
...
PTR_SEEDED_UNK_F_WRAM: ; 1E:03D5, 0x03C3D5
    JSR ENGINE_WRAM_ARR_MOD_LOOPS_UNK ; Mod ??
PTR_SEEDED_UNK_G_WRAM: ; 1E:03D8, 0x03C3D8
    LDA #$48 ; Seed ??
    LDX #$92
    JMP STORE_PACKET_PTR_AND_LATCH_QUEUE_UPDATE_CREATE?
PTR_SEEDED_UNK_H_GFX_UPDATE: ; 1E:03DF, 0x03C3DF
    LDA #$6A ; Seed 13:136A
    LDX #$93
    JMP STORE_PACKET_PTR_AND_LATCH_QUEUE_UPDATE_CREATE?
...
LIB_OBJECTS_AND_SETTLE_AND_FLAGS_UNK: ; 1E:03F4, 0x03C3F4
    ...
    LDA #$01
    STA NMI_FLAG_EXECUTE_UPDATE_BUF_AND_MORE_TODO ; Execute.
    LDA #$00
...
LOOP_STACK_POSITIVE: ; 1E:040F, 0x03C40F
    PHA ; Save it.
    TAX ; Val to index.
    LDA SCRIPT_PARTY_ATTRIBUTES[32],X ; Load at index.
    BEQ INDEXED_LOAD_EQ_0x00 ; == 0, goto.
    LDA PARTY_ATTR_PTR[2],X ; Move from arr to fptr for packeting.
    STA FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]
    LDA PARTY_ATTR_PTR+1,X
    STA FPTR_PACKET_CREATION/PTR_H_FILE_IDK+1
    LDY #$01 ; Stream index.
    LDA SCRIPT_PARTY_ATTRIBUTES+1,X ; Load from arr.
    AND LIB_BCD/EXTRA_FILE_BCD_A ; And with ??
...
STREAM_LT_0x18: ; 1E:042B, 0x03C42B
    LDA SCRIPT_PARTY_ATTRIBUTES+3,X ; Load from.
...
LIB_PACKET_DISPLAY_TODO: ; 1E:046E, 0x03C46E
    ...
    ASL A
    ASL A
    STA PACKET_CREATION_STREAM_INDEX ; Store to.
    LDA GFX_COORD_HORIZONTAL_OFFSET ; Load.
    ADC #$01 ; Add, no carry here because of AND and shifts.
    LSR A ; >> 1, /2.
    ORA PACKET_CREATION_STREAM_INDEX ; Or with.
    STA PACKET_CREATION_STREAM_INDEX ; Store result.
    TAY ; To Y index.
...
ARRAY_SET_LARGE: ; 1E:048C, 0x03C48C
    ...
    ADC #$01 ; Compliment.
    CLC ; Prep add.
    ADC PACKET_CREATION_STREAM_INDEX ; Add with.
    TAY ; To Y index.
...
LOOP_PACKETY: ; 1E:04E5, 0x03C4E5
    ...
    AND #$03 ; Keep lower.
    STA PACKET_CREATION_STREAM_INDEX ; Store.
    INY ; Array++
    ...
    ORA PACKET_CREATION_STREAM_INDEX ; Combine with.
    STA PACKET_CREATION_STREAM_INDEX ; Store result.
    INY ; Array++
    LDA WRAM_PAGE_LARGE_UNK[384],Y ; Load array.
...
SLOT_INDEX_NE_0x14: ; 1E:055B, 0x03C55B
    ...
    PHA
    LDY #$01 ; Seed index.
    LDA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Load from index.
...
VAL_LT_0x4: ; 1E:05C9, 0x03C5C9
    ...
    BCS DATA_RET_OTHER
    STA WRAM_DATA_UNK[3],X ; Store to arr.
    INX ; Index++
    CPX #$03 ; If _ #$03
...
STREAM_PAGE_Y_INDEXED_CC_0x01-0x05: ; 1E:062E, 0x03C62E
    ...
    LDA WRAM_ARR_PARTY_CHARACTER_IDS?[6],Y ; Load data indexed.
    BEQ RTS ; == 0, return CS.
...
DATA_MOVE_AND_MOD_WITH_PTR_UNK: ; 1E:0637, 0x03C637
    ...
    INY
    RTS
CREATE_PTR_FROM_PASSED_TO_BANKED/WRAM_DATA: ; 1E:0665, 0x03C665
    STA LIB_BCD/EXTRA_FILE_BCD_B ; Store arg. AAAA.AABB, PTRH = 0x74 + A, PTRL = B << 6
    LDA #$00 ; Clear for shifts low.
    LSR LIB_BCD/EXTRA_FILE_BCD_B ; Shift bit off.
    ROR A ; Into A.
    LSR LIB_BCD/EXTRA_FILE_BCD_B ; 2x
    ROR A
    ADC #$00 ; Carry to bit 0, fancy. But always 0x00 here.
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Store to, addr L.
    LDA LIB_BCD/EXTRA_FILE_BCD_B ; Load passed val 00AA.AAAA
    ADC #$74 ; Add with, PTR offset base. Range of 0x74-B3, slot size 0x40.
    STA LIB_BCD/EXTRA_FILE_BCD_B ; Store addr H.
    RTS
...
ENGINE_CREATE_UPDATE_BUF_INIT_INC?: ; 1E:06D2, 0x03C6D2
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    LDX #$00
    STX NMI_PPU_CMD_PACKETS_INDEX ; Reset index to trigger.
    BEQ ALT_ENTRY ; Always taken here.
ENGINE_CREATE_UPDATE_BUF_INIT_DEC?: ; 1E:06DB, 0x03C6DB
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    LDX #$00
    STX NMI_PPU_CMD_PACKETS_INDEX ; Reset index to trigger.
    JSR SETUP_CREATION_LOWER_VOFFSET ; Up row.
    JSR ENGINE_PACKET_FINALIZATION_HELPER ; Finalize it.
ALT_ENTRY: ; 1E:06E8, 0x03C6E8
    ...
    LDA #$80
    STA NMI_FLAG_EXECUTE_UPDATE_BUF_AND_MORE_TODO ; Set flag to trigger, too.
...
LIB_SUB_HELPER_MOVE_CMD_TO_DEEPER: ; 1E:07AF, 0x03C7AF
    CMP #$01 ; If _ #$01
    BNE RTS ; !=, leave.
    PHA ; Save it.
    LDX #$A4 ; Seed index.
...
ENGINE_PACKET_CREATE_FROM_FILE: ; 1E:083D, 0x03C83D
    ...
    LDA ENGINE_PACKINATOR_ARG_SEED_BLANK_PRE_COUNT ; Move count?
...
SETUP_CREATION_LOWER_VOFFSET: ; 1E:086D, 0x03C86D
    DEC GFX_COORD_VERTICAL_OFFSET ; --
    JSR PACKET_CALC_COORDS_TO_VADDR_HELPER ; Calc pos.
    LDA ENGINE_PACKINATOR_ARG_SEED_BLANK_PRE_COUNT ; Move to local.
...
LAUNCH_PACKET_UPDATE_SCRIPTS: ; 1E:08A1, 0x03C8A1
    STA ENGINE_SCRIPT_SWITCH_VAL_CONTINUE_UPDATES? ; Val to.
...
TABLE_PTR_H: ; 1E:08AE, 0x03C8AE
    LOW(SCRIPT_RTN_A_UPDATE_FPTR_THINGS) ; PPU update switch.
TABLE_PTR_L: ; 1E:08AF, 0x03C8AF
    HIGH(SCRIPT_RTN_A_UPDATE_FPTR_THINGS) ; TODO: All these functions specifically.
    LOW(SCRIPT_RTN_A_UPDATE_FPTR_THINGS)
    HIGH(SCRIPT_RTN_A_UPDATE_FPTR_THINGS)
    LOW(SCRIPT_RTN_A_UPDATE_FPTR_THINGS)
    HIGH(SCRIPT_RTN_A_UPDATE_FPTR_THINGS)
    LOW(SCRIPT_RTN_A_UPDATE_FPTR_THINGS)
    HIGH(SCRIPT_RTN_A_UPDATE_FPTR_THINGS)
    LOW(SCRIPT_RTN_B_FILE_COORDS_AND_UPDATE_SOLVE)
    HIGH(SCRIPT_RTN_B_FILE_COORDS_AND_UPDATE_SOLVE)
    LOW(SCRIPT_RTN_C_STREAM_GOSUB_SAVE) ; Swap to other stream from stream.
    HIGH(SCRIPT_RTN_C_STREAM_GOSUB_SAVE)
    LOW(SCRIPT_RTN_D_FILE_TILE_OR_DEFAULT_FILE_COUNT) ; Do tile county.
    HIGH(SCRIPT_RTN_D_FILE_TILE_OR_DEFAULT_FILE_COUNT)
    LOW(SCRIPT_RTN_E_FILE_VALS_AND_DISPLAY_THINGY?)
    HIGH(SCRIPT_RTN_E_FILE_VALS_AND_DISPLAY_THINGY?)
    LOW(SCRIPT_RTN_F_STREAM_GOSUB_FILE)
    HIGH(SCRIPT_RTN_F_STREAM_GOSUB_FILE)
    LOW(SCRIPT_RTN_G_RELAUNCH)
    HIGH(SCRIPT_RTN_G_RELAUNCH)
SCRIPT_RTN_F_STREAM_GOSUB_FILE: ; 1E:08C2, 0x03C8C2
    LDY PACKET_CREATION_STREAM_INDEX ; Stream reload.
SCRIPT_RTN_F_STREAM_GOSUB_FILE_SEEDED_STREAM_INDEX: ; 1E:08C4, 0x03C8C4
    LDA [FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]],Y ; Replacement pointer from stream.
    PHA ; Saved to stack for var efficiency.
    INY ; Stream++
    LDA [FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]],Y ; Move PTR H.
    STA FPTR_PACKET_CREATION/PTR_H_FILE_IDK+1
    PLA
    STA FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2] ; Ptr L.
    LDY #$00 ; Reset stream.
    JMP [ENGINE_PTR_PACKET_MANAGER[2]] ; Rehandle it.
SCRIPT_RTN_C_STREAM_GOSUB_SAVE: ; 1E:08D4, 0x03C8D4
    LDY PACKET_CREATION_STREAM_INDEX ; Load.
    BEQ SCRIPT_RTN_A_UPDATE_FPTR_THINGS ; Stream == 0x00, do A.
    LDA FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2] ; Save FPTR making.
    PHA
    LDA FPTR_PACKET_CREATION/PTR_H_FILE_IDK+1
    PHA
    TYA ; Save stream index to stack.
    PHA
    BNE SCRIPT_RTN_F_STREAM_GOSUB_FILE_SEEDED_STREAM_INDEX ; Always taken.
SCRIPT_RTN_A_UPDATE_FPTR_THINGS: ; 1E:08E2, 0x03C8E2
    PLA ; Pull val.
    BEQ STACK_CONFIG_0x00 ; == 0, goto.
    TAY ; Value to Y.
    INY ; Y += 2. TODO why.
    INY
    PLA ; Pull behind stack. Fptr to make.
...
STACK_CONFIG_0x00: ; 1E:08F1, 0x03C8F1
    ...
    BEQ PAST_INDEX_IS_EOF ; == 0, goto.
    TXA ; X to Y index.Else X to Y instead.
    TAY
PAST_INDEX_IS_EOF: ; 1E:08FA, 0x03C8FA
    LDA ENGINE_SCRIPT_SWITCH_VAL_CONTINUE_UPDATES? ; Load switch.
    BPL FORWARD_FPTR_PORTION ; Positive, forward it.
    INC GFX_COORD_VERTICAL_OFFSET ; ++ if negative.
    RTS ; Leave.
FORWARD_FPTR_PORTION: ; 1E:0901, 0x03C901
    LDA PACKET_CREATION_STREAM_INDEX ; Load ??
    JSR FORWARD_PACKET_CREATION_FP_BY_A ; Forward by amount.
    LDA ENGINE_SCRIPT_SWITCH_VAL_CONTINUE_UPDATES? ; Load val.
    RTS ; Leave.
SCRIPT_RTN_B_FILE_COORDS_AND_UPDATE_SOLVE: ; 1E:0909, 0x03C909
    LDY PACKET_CREATION_STREAM_INDEX ; Load stream.
    LDA [FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]],Y ; File to coords.
    ...
    INY ; Stream++
    STY PACKET_CREATION_STREAM_INDEX ; Stream save.
    BCC CC_SKIP_DEC ; TODO: Always taken?
    DEC GFX_COORD_VERTICAL_OFFSET ; On CS, VCoord-- ??
CC_SKIP_DEC: ; 1E:091B, 0x03C91B
    ...
    LDX NMI_PPU_CMD_PACKETS_INDEX ; Seed index in X instead for store.
PAST_NONZERO: ; 1E:0924, 0x03C924
    JSR PACKET_CALC_COORDS_TO_VADDR_HELPER ; Calc pos.
    JSR PACKET_HEADER_HELPER_TO_EDGE_OF_SCREEN ; Header it.
    LDY PACKET_CREATION_STREAM_INDEX ; Reload creation fptr stream pos.
    JMP [ENGINE_PTR_PACKET_MANAGER[2]] ; Go back to handler.
SCRIPT_RTN_D_FILE_TILE_OR_DEFAULT_FILE_COUNT: ; 1E:092F, 0x03C92F
    LDY PACKET_CREATION_STREAM_INDEX ; Load index.
    LDA [FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]],Y ; Load from file.
    BCC USE_FILE_DATA ; CC, goto. Use file.
    LDA #$A0 ; Seed. To append, blank?
USE_FILE_DATA: ; 1E:0937, 0x03C937
    STA ENGINE_SCRIPT_SWITCH_VAL_CONTINUE_UPDATES? ; Store, to append with.
    INY ; Stream++
    LDA [FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]],Y ; Load from file, count todo.
    INY ; Stream++
    STY PACKET_CREATION_STREAM_INDEX ; Save stream pos.
    TAY ; Val to Y index. Count.
LOOP_DO: ; 1E:0940, 0x03C940
    DEY ; Y--
    BMI EXIT_RELAUNCH ; Negative, done.
    LDA ENGINE_SCRIPT_SWITCH_VAL_CONTINUE_UPDATES? ; Load val.
    JSR APPEND_DATA_TO_UPDATE_BUF ; Append to packet.
    JMP LOOP_DO ; Loop on Y.
EXIT_RELAUNCH: ; 1E:094B, 0x03C94B
    LDY PACKET_CREATION_STREAM_INDEX ; Restore.
    JMP [ENGINE_PTR_PACKET_MANAGER[2]] ; And relaunch.
SCRIPT_RTN_E_FILE_VALS_AND_DISPLAY_THINGY?: ; 1E:0950, 0x03C950
    LDY PACKET_CREATION_STREAM_INDEX ; Stream index.
...
VAL_POSITIVE: ; 1E:0975, 0x03C975
    ...
    STA LIB_BCD/EXTRA_FILE_BCD_A,Y ; Store to 0x0060.
    DEY ; Y--
    BPL VAL_POSITIVE ; Positive, goto.
    TXA ; Save X to stack.
    PHA ; Save X.
    JSR ENGINE_24BIT_TO_TEXT_DIGITS ; Do to text.
    PLA ; Pull.
    TAX ; Val to X.
...
ALT_0x00: ; 1E:09A3, 0x03C9A3
    LDY #$00 ; Stream index.
ALT_COUNT_NONZERO: ; 1E:09A5, 0x03C9A5
    LDA ENGINE_SCRIPT_SWITCH_VAL_CONTINUE_UPDATES? ; Load ??
...
ENGINE_DATA_DICT_MOD_A_UNK: ; 1E:0A5C, 0x03CA5C
    INY ; Stream++
    STY PACKET_CREATION_STREAM_INDEX ; Save.
    CMP #$40 ; If _ #$40
    BCS EXIT_CS ; >=, goto.
    TAY ; Stream to Y.
    LDA DICTIONARY_ALT_VALS,Y ; Load data.
    LDY PACKET_CREATION_STREAM_INDEX ; Restore stream.
    CMP #$80 ; If alt _ #$80
    BCS EXIT_CS ; >=, leave.
    CMP #$20 ; If alt _ #$20
...
ENGINE_DATA_DICT_MOD_B_UNK: ; 1E:0A7C, 0x03CA7C
    INY ; Stream++
    STY PACKET_CREATION_STREAM_INDEX ; Save stream as Y might be used. Why not do this later. :(
    CMP #$40 ; If _ #$40
    BCS EXIT_A:0xA0_CS ; >=, goto.
    TAY ; Val to Y index.
    LDA DICTIONARY_ALT_VALS,Y ; Load alt.
    LDY PACKET_CREATION_STREAM_INDEX ; Restore stream index.
    CMP #$80 ; If _ #$80
...
VAL_POSITIVE: ; 1E:0AA9, 0x03CAA9
    CMP #$7F ; If _ #$7F
    BCS SETTLE_FOR_PPU_READ ; >=, goto. 0x7F = Settle.
    ...
    LDA #$18 ; New R6.
    LDX #$06 ; Seed R6.
    JSR ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Set R6.
    LDY #$00 ; Stream reset.
    LDA [FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]],Y ; Load from stream.
    STA PPU_LOCATION_FOCUS_VAL_24B[3] ; Store to ??
    INY ; Stream++
    LDA [FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]],Y ; Load from stream.
    STA PPU_LOCATION_FOCUS_VAL_24B+1 ; Store to ??
    INY ; Stream++
    LDA [FPTR_PACKET_CREATION/PTR_H_FILE_IDK[2]],Y ; Load from stream.
    STA PPU_LOCATION_FOCUS_VAL_24B+2 ; Store to ??
    PLA ; Pull R6 saved.
    JSR ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Set R6 again.
SETTLE_FOR_PPU_READ: ; 1E:0AE1, 0x03CAE1
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    LDA #$0A ; Packet type, PPU read.
    STA NMI_PPU_CMD_PACKETS_BUF[69]
    LDA PPU_LOCATION_FOCUS_VAL_24B+2 ; Load ??
    LSR A ; >> 1, /2.
    LDA PPU_LOCATION_FOCUS_VAL_24B+1 ; Load ??
    ROR A ; Rotate into.
    SEC ; Prep CS.
    ROR A ; Into A.
    ...
    LDA PPU_LOCATION_FOCUS_VAL_24B[3] ; Load ??
...
PTR_POSITIVE: ; 1E:0B21, 0x03CB21
    ...
    SBC #$10 ; -= 0x10
    CLC ; Prep add.
    ADC PPU_LOCATION_FOCUS_VAL_24B[3] ; Add with.
    STA PPU_LOCATION_FOCUS_VAL_24B[3] ; Store to.
    LDA #$00 ; Carry seed.
    ADC PPU_LOCATION_FOCUS_VAL_24B+1 ; Carry add.
    STA PPU_LOCATION_FOCUS_VAL_24B+1 ; Store val.
    LDA #$00 ; Carry seed again.
    ADC PPU_LOCATION_FOCUS_VAL_24B+2 ; Carry add.
    STA PPU_LOCATION_FOCUS_VAL_24B+2 ; Store val.
    RTS ; Leave.
...
STATES_COMBINED_ANY_SET_HANDLER: ; 1E:0BEB, 0x03CBEB
    ...
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    LDA SOUND_MAIN_SONG_CURRENTLY_PLAYING_ID ; Load ??
    PHA ; Save it.
...
VAL_NEGATIVE: ; 1E:0CB1, 0x03CCB1
    ...
    LDA #$02
    STA SOUND_EFFECT_REQUEST_ARRAY[5] ; Store to sound ??
    LDA #$01
...
MENU_DOUBLE_EXIT_SAVE_SONG_AND_ASSIGN_NEW_AND_SFX: ; 1E:0CD8, 0x03CCD8
    LDA SOUND_MAIN_SONG_CURRENTLY_PLAYING_ID ; Save value.
    PHA
    LDA #$FF
    STA R_**:$000F ; Set ??
    ...
    LDA #$01
    STA SOUND_EFFECT_REQUEST_ARRAY+4 ; Set sound effect creepy boss/teleport screech.
...
LOOP_NONZERO: ; 1E:0D26, 0x03CD26
    ...
    JSR SLOTS_AND_FPTRS_TODO_LARGER_FILES ; Do slotty.
    PLA ; Pull todo why.
    ...
    JSR LIB_FIND_PARTY_ID_0x7 ; Do checks.
    LDX #$14
...
ACTION_INDEX_STORE_AND_RETURN_VALUE_UNK: ; 1E:0DAF, 0x03CDAF
    ...
    STA ACTION_BUTTONS_RESULT ; Store to, result.
    RTS ; Leave.
...
ENGINE_MAIN_SONG_REPLACE_AND_THEN_RESTORE_MAIN_SONG?: ; 1E:0DE4, 0x03CDE4
    ...
    TAX ; To index.
    PLA ; Pull passed.
    JSR SOUND_ASSIGN_NEW_MAIN_SONG ; Assign as main.
    LDA #$00
    STA CONTROL_ACCUMULATED?[2] ; Clear accumulated.
...
BUTTON_B_PRESSED: ; 1E:0DFA, 0x03CDFA
    ...
    TXA ; Original song.
    JMP SOUND_ASSIGN_NEW_MAIN_SONG ; Play it again.
FILE_FROM_PPU_READ_2KB_TO_WRAM_6C00-7EFF: ; 1E:0E02, 0x03CE02
    LDY #$5E ; Seed GFX Bank for data.
    LDA #$00
    LDX #$6C ; Seed 0x6C00?
FPTR_SEEDED_SETTLE_AND_DISP_UPDATES_TODO: ; 1E:0E08, 0x03CE08
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Set fptr 0x6C00/0x6000
    STX LIB_BCD/EXTRA_FILE_BCD_B
    JSR ENGINE_SETTLE_ALL_UPDATES?
    TYA ; Val to A.
    LDX #$01 ; R1
    JSR ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Set GFX Bank.
...
LOOP_TO_TARGET: ; 1E:0E40, 0x03CE40
    ...
    STA [LIB_BCD/EXTRA_FILE_BCD_A],Y ; Store to fp. 0x6C00 - 0x73FF.
    INY ; Index++
    CPY #$40 ; If _ #$40
    ...
    CLC ; Prep add.
    TYA ; A = 0x40
    ADC LIB_BCD/EXTRA_FILE_BCD_A ; Add consumed to ptr.
    STA LIB_BCD/EXTRA_FILE_BCD_A
    LDA #$00
    ADC LIB_BCD/EXTRA_FILE_BCD_B ; Carry add.
    STA LIB_BCD/EXTRA_FILE_BCD_B
    CLC ; Prep add.
    TYA ; Consumed to A.
...
PPU_READ_INTO_$0110_HELPER_LOOP_UNK: ; 1E:0E6D, 0x03CE6D
    ...
    LDX #$00
    STA LIB_BCD/EXTRA_FILE_BCD_A
    STX LIB_BCD/EXTRA_FILE_BCD_B ; Set $0000
    LDA #$00
    LDX #$20
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2]
    STX SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Set to $2000.
    LDA #$10
    STA RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY[8] ; Set loop count, 0x10.
LOOPS_NONZERO: ; 1E:0E81, 0x03CE81
    LDA LIB_BCD/EXTRA_FILE_BCD_A ; Seed PPU Addr H.
    LDX LIB_BCD/EXTRA_FILE_BCD_B ; Seed L.
    LDY #$09 ; Seed type, read to RAM.
    JSR ENGINE_LARGE_UNIQUE_PPU_PACKET_HELPER ; Do.
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Seed PPU Addr H.
    LDX SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Seed L.
    ...
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Store result.
    LDA #$00 ; Carry seed.
    ADC SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Add with.
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Store result.
    DEC RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY[8] ; Loops--
...
ENGINE_HELPER_R7_0x13: ; 1E:0ED3, 0x03CED3
    LDA #$13
    LDX #$07
    JMP ENGINE_MAPPER_COMMIT_BANK_X_VAL_A
...
ENGINE_HELPER_R6_0x14: ; 1E:0EE1, 0x03CEE1
    LDA #$14 ; R6 = 0x14
    LDX #$06
    JMP ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Do, return.
ENGINE_SET_GFX_BANKS_FPTR_AX: ; 1E:0EE8, 0x03CEE8
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Set PTR.
    STX LIB_BCD/EXTRA_FILE_BCD_B
...
STREAM_INDEX_POSITIVE: ; 1E:0EF0, 0x03CEF0
    ...
    JSR ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Set bank 0x05 -> 0x00
...
STREAM_POSITIVE: ; 1E:0F2A, 0x03CF2A
    STA ENGINE_COMMIT_PALETTE[32],Y ; Store val to palette.
    DEY ; Index--
    BPL INDEX_POSITIVE ; Positive, loop.
    LDY #$0F ; Reset index.
INDEX_POSITIVE: ; 1E:0F32, 0x03CF32
    LDA PALETTE_SPRITE_UNK,Y ; Load from arr.
    STA ENGINE_COMMIT_PALETTE+16,Y ; Store to sprite palette.
    DEY ; Index--
    BPL INDEX_POSITIVE ; Positive, goto.
    LDX ENGINE_COMMIT_PALETTE+12 ; Load ??
    LDY ENGINE_COMMIT_PALETTE+14
    STX R_**:$0017 ; Save to.
    STY R_**:$0016
    LDX #$0F ; Seed ??
    LDY #$30
    STX ENGINE_COMMIT_PALETTE+12
    STY ENGINE_COMMIT_PALETTE+14
...
VALS_EQ: ; 1E:1132, 0x03D132
    LDA R_**:$0094 ; Load.
    LDX #$06 ; R6.
    JSR ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Set R6 bank.
    CLC ; Prep add.
...
VAL_EQ_0x00: ; 1E:115E, 0x03D15E
    ...
    LDX #$07 ; Set R7.
    JSR ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Set R7.
...
WAIT_UPDATE: ; 1E:121C, 0x03D21C
    LDA NMI_FLAG_EXECUTE_UPDATE_BUF_AND_MORE_TODO ; Wait on.
...
SCRIPT_R6/R7_PAIRED_AND_STREAMS_UNK: ; 1E:14B7, 0x03D4B7
    ...
    ORA #$01 ; Set bottom.
    LDX #$06 ; Set as R6.
    JSR ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Bank.
    LDA SCRIPT_R7_UNK ; Load.
    LSR A ; >> 1, /2.
    ORA #$01 ; Set bottom.
    LDX #$07 ; Set as R7.
...
CREATE_B800_PTR_HELPER: ; 1E:159D, 0x03D59D
    ...
    ORA #$01 ; Set bit.
    LDX #$07 ; Set R7.
    JSR ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Bank R7 in.
    LDA SCRIPT_USE_UNK_C_PTR_H ; Load.
    LSR A ; >> 2, /4.
    LSR A
    AND #$07 ; Keep lower.
...
GFX_BANK_R2_SET: ; 1E:1612, 0x03D612
    ...
    LDX #$02 ; R2 seed.
    JSR ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Set GFX.
    STA SCRIPT_R6_UNK/R2_GFX_BANK_UNK ; Store val to.
    AND #$03 ; Keep lower.
...
HUGE_ASS_STREAMS_THINGY_IDFK: ; 1E:1674, 0x03D674
    ...
    LDA WRAM_6784_UNK ; Load ??
    AND #$C0 ; Keep 1100.0000
    STA SCRIPT_LOADED_SHIFTED_UNK[2] ; Store to.
    LDA WRAM_6785_UNK ; Move ??
    STA SCRIPT_USE_UNK_A
...
X_LOOP_TO_TARGET: ; 1E:16DA, 0x03D6DA
    LDA WRAM_ARR_PARTY_CHARACTER_IDS?[6],X ; Load indexes.
    BEQ SKIP_UNK ; == 0, goto.
...
NONZERO_ALT: ; 1E:171B, 0x03D71B
    ...
    LDY #$19 ; Stream index.
    LDA ROUTINE_SWITCH_ID_TODO ; Move to stream.
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y
    EOR #$04 ; Invert bit.
    STA ROUTINE_SWITCH_ID_TODO ; Store val.
    JSR MOVE_TO_STREAM_UNK ; Do.
...
LIB_RETURN_INDEX_OF_PARTY_MEMBER_ID_A: ; 1E:178D, 0x03D78D
    LDX #$00 ; Reset index.
INDEX_LT_0x4: ; 1E:178F, 0x03D78F
    CMP WRAM_ARR_PARTY_CHARACTER_IDS?[6],X ; Compared passed to.
...
LOOPY: ; 1E:179D, 0x03D79D
    ...
    STA WRAM_ARR_PARTY_CHARACTER_IDS?[6],X ; Move slot down.
    INX ; X++
    BCC LOOPY ; Always taken.
CLEAR_SLOT_IDFK: ; 1E:17AC, 0x03D7AC
    LDA #$00 ; Clear.
STORE_SLOT: ; 1E:17AE, 0x03D7AE
    STA WRAM_ARR_PARTY_CHARACTER_IDS?[6],X ; Store.
    TXA ; Slot to A.
...
LIB_FPTR_AND_MOVE_HELPER: ; 1E:18BA, 0x03D8BA
    ...
    RTS ; Leave.
LIB_FIND_PARTY_ID_0x7: ; 1E:18C9, 0x03D8C9
    LDA #$07 ; Find ID 0x7.
    JSR LIB_RETURN_INDEX_OF_PARTY_MEMBER_ID_A ; Find it.
LIB_FIND_PARTY_ID_0x6: ; 1E:18CE, 0x03D8CE
    LDA #$06 ; Find ID 0x6.
    JMP LIB_RETURN_INDEX_OF_PARTY_MEMBER_ID_A ; Find it.
LIB_SPECIAL_CHECKS_UNK: ; 1E:18D3, 0x03D8D3
    LDA CURRENT_SAVE_MANIPULATION_PAGE+385 ; Load ??
    BPL POSITIVE_VAL ; Positive, goto.
    LDA #$06 ; Val ??
    JSR LIB_RETURN_INDEX_OF_PARTY_MEMBER_ID_A ; Do a bit, slotty.
...
VAL_POSITIVE: ; 1E:18E7, 0x03D8E7
    LDX #$00
    STX SLOT/DATA_OFFSET_USE/CURR? ; Set slot.
...
LOADED_ZERO: ; 1E:190E, 0x03D90E
    ...
    LDA #$01
    STA SLOT/DATA_OFFSET_USE/CURR? ; Set ??
    LDA #$00
...
DATA_WRITING_UNK: ; 1E:196F, 0x03D96F
    ...
    .db 40
    .db DB
FOCUS/ID_RELATED_TODO: ; 1E:1977, 0x03D977
    DEC SAVE_ID_FOCUS/OTHER ; -- ??
    BMI RTS ; Negative, goto.
    BEQ RTS ; == 0, leave.
LOOP_CHECK: ; 1E:197D, 0x03D97D
    LDA WRAM_ARR_PARTY_CHARACTER_IDS?[6] ; Load.
    CMP #$01 ; If _ #$01
    BEQ VAL_EQ_0x1 ; ==, goto.
...
VAL_LT_0x11: ; 1E:19B9, 0x03D9B9
    ...
    LDA SLOT/DATA_OFFSET_USE/CURR? ; Load ??
    ADC #$20 ; += 0x20
    STA SLOT/DATA_OFFSET_USE/CURR? ; Store result.
    INX ; Index++
    CPX SAVE_ID_FOCUS/OTHER ; If _ var
    BCC LIB_SWAP_UNK ; <, goto.
    JMP ENGINE_WRAM_STATE_WRITE_DISABLED
OFFSET_ADD_AND_SWAP_UNK: ; 1E:19DE, 0x03D9DE
    CLC ; Prep add.
    ADC SLOT/DATA_OFFSET_USE/CURR? ; Add ??
    TAY ; To Y index.
    LDA WRAM_6780_BASE_FOR_ATTR_FETCH/RTN?,Y ; Load ??
    PHA ; Save val.
    LDA R_**:$67A0,Y ; Swap vals.
    STA WRAM_6780_BASE_FOR_ATTR_FETCH/RTN?,Y
    PLA ; Pull val.
    STA R_**:$67A0,Y ; Store val.
    RTS
LIB_FIND_SLOT_OCCUPIED_AND_BETWEEN_0x1-0x5: ; 1E:19F1, 0x03D9F1
    SEC ; Prep ret CS, fail.
    LDA WRAM_ARR_PARTY_CHARACTER_IDS?[6],X ; Load party at index.
    BEQ FAIL_IS_CLEAR ; == 0, goto. Ret CS.
    CMP #$06 ; Slot _ #$06. <, ret CC. >=, ret CS.
FAIL_IS_CLEAR: ; 1E:19F9, 0x03D9F9
    RTS ; 0x01-0x05, CC. CS otherwise.
SLOTS_AND_FPTRS_TODO_LARGER_FILES: ; 1E:19FA, 0x03D9FA
    LDX #$00 ; Index reset.
SLOT_FIND_LOOP: ; 1E:19FC, 0x03D9FC
    LDA WRAM_ARR_PARTY_CHARACTER_IDS?[6],X ; Load from indexed.
    BEQ TO_NEXT_ARRAY_ID ; ==, goto.
    JSR CREATE_PTR_FROM_PASSED_TO_BANKED/WRAM_DATA ; Make ptr ??
    LDY #$2C ; Index.
...
TO_NEXT_ARRAY_ID: ; 1E:1A10, 0x03DA10
    INX ; Index++
    CPX #$04 ; If _ #$04
    BCC SLOT_FIND_LOOP ; <, loop.
    RTS ; Leave.
...
LIB_UNK: ; 1E:1A48, 0x03DA48
    LDA SLOT/DATA_OFFSET_USE/CURR? ; Move ??. If 0x00, loops forever to crash game.
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2]
    LDA TRIO_FILE_OFFSET_UNK[3] ; Move to misc use. ??
    STA LIB_BCD/EXTRA_FILE_BCD_A
    LDA TRIO_FILE_OFFSET_UNK+1
    STA LIB_BCD/EXTRA_FILE_BCD_B
    LDA TRIO_FILE_OFFSET_UNK+2
    STA LIB_BCD2/EXTRA_FILE_STREAM_INDEX
...
VAL_EQ_0x00: ; 1E:1A61, 0x03DA61
    CLC ; Prep add.
    ADC LIB_BCD/EXTRA_FILE_BCD_A ; Add val to.
    STA TRIO_FILE_OFFSET_UNK[3] ; Store result.
    LDA #$00 ; Carry seed.
    ADC LIB_BCD/EXTRA_FILE_BCD_B ; Add carry.
    STA TRIO_FILE_OFFSET_UNK+1 ; Store result.
    LDA #$00 ; Seed carry.
    ADC LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; Add carry.
    STA TRIO_FILE_OFFSET_UNK+2 ; Store result.
    JSR ENGINE_WRAM_STATE_WRITEABLE ; WRAM writable.
    LDX #$00 ; Index set.
VAL_LT_0x4: ; 1E:1A77, 0x03DA77
    JSR LIB_FIND_SLOT_OCCUPIED_AND_BETWEEN_0x1-0x5 ; Do ??
    BCS RTN_RET_CS ; Ret CS, goto.
    STA PARTY/BATTLE_ID?_TODO ; Set ??
    TXA ; X to A.
    LSR A ; >> 1, /2.
    ROR A ; Rotate into now.
    ROR A
    ROR A
    STA SCRIPT_BATTLE_PARTY_ID_FOCUS ; Store shifted.
    TXA ; X to A again.
    PHA ; Save it.
    JSR LIB_FPTR_AND_MOVE_HELPER ; Do lib.
    LDY #$01 ; Stream index.
    ...
    LDY #$11 ; Stream index.
    CLC ; Prep add.
    LDA TRIO_FILE_OFFSET_UNK[3] ; Delta.
    ADC [R6_BANKED_ADDR_MOVED[2]],Y ; Add with stream.
    STA [R6_BANKED_ADDR_MOVED[2]],Y ; Store result.
    INY ; Stream++
    LDA TRIO_FILE_OFFSET_UNK+1 ; Delta.
    ADC [R6_BANKED_ADDR_MOVED[2]],Y ; Add with stream.
    STA [R6_BANKED_ADDR_MOVED[2]],Y ; Store result.
    INY ; Stream++
    LDA TRIO_FILE_OFFSET_UNK+2 ; Delta.
    ADC [R6_BANKED_ADDR_MOVED[2]],Y ; Add with stream.
    STA [R6_BANKED_ADDR_MOVED[2]],Y ; Store result.
...
NO_CARRY_FINAL: ; 1E:1AB9, 0x03DAB9
    ...
    INY ; Stream++
    LDA [R6_BANKED_ADDR_MOVED[2]],Y ; Load ??
    SBC SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; -= ??, carried.
    INY ; Stream++
    LDA [R6_BANKED_ADDR_MOVED[2]],Y ; Load ??
    SBC ALT_STUFF_COUNT? ; -= ??, carried.
...
CC_LOOP: ; 1E:1B0A, 0x03DB0A
    ASL PAIR_UNK_LEVEL_UP_RELATED?[2] ; << var.
    ASL A ; << 1.
    BCC CC_LOOP ; CC, do more.
    JSR RANDOMIZE_GROUP_A ; Do adds.
    AND PAIR_UNK_LEVEL_UP_RELATED?[2] ; Remove ??
...
INDEX_LT_0x4: ; 1E:1B1B, 0x03DB1B
    JSR LIB_FIND_SLOT_OCCUPIED_AND_BETWEEN_0x1-0x5 ; Do ??
    BCS RET_CS ; CS, goto.
    STA PARTY/BATTLE_ID?_TODO ; Store loaded from arr.
    TXA ; Save X.
    PHA
    JSR BANKED_DERP ; Do ??
    PLA ; Restore X.
...
LIB_IDFK_DECIMALY_AND_IDK: ; 1E:1B40, 0x03DB40
    TAX ; A to X.
    INX ; ++
    STX SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Store ??
    INX ; ++
    STX LIB_BCD/EXTRA_FILE_BCD_A ; Store ??
    LDA #$00
    STA LIB_BCD/EXTRA_FILE_BCD_B ; Clear ??
    STA LIB_BCD2/EXTRA_FILE_STREAM_INDEX
    JSR LIB_MATH_24BIT_MULTIPLY? ; Do.
    JSR LIB_MATH_24BIT_MULTIPLY? ; Again. Mistake?
    JSR LIB_SETUP_PTR_AND_R6_UNK ; Do ??
    LDY #$00 ; Stream index.
    LDA [RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY[8]],Y ; Move ??
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2]
    JSR LIB_MATH_24BIT_MULTIPLY? ; Do RTN.
    LDA LIB_BCD/EXTRA_FILE_BCD_B ; Move ??
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2]
    LDA LIB_BCD2/EXTRA_FILE_STREAM_INDEX ; 2x
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1
...
VAL_LT_0x8: ; 1E:1B93, 0x03DB93
    ...
    LSR A
    CLC ; Prep add.
    ADC [RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY[8]],Y ; Add with ptr.
    LSR A ; >> 1, /2.
...
NO_OVERFLOW: ; 1E:1BED, 0x03DBED
    ...
    LDA PARTY/BATTLE_ID?_TODO ; Load val.
    CMP #$03 ; If _ #$03
    BCS EXIT_CC ; >=, goto.
    LDY #$0F ; Stream index.
    LDA [R6_BANKED_ADDR_MOVED[2]],Y ; Load from file.
    STA LIB_BCD/EXTRA_FILE_BCD_A ; Store to misc.
    LSR A ; >> 1, /2.
    CLC ; Prep carry.
    LDY #$05 ; Stream index ??
...
STATS_THINGY_LARGE: ; 1E:1C3F, 0x03DC3F
    ...
    TAX ; To X index.
    INY ; Stream++
    LDA LIB_BCD/EXTRA_FILE_BCD_B ; Load from stream.
    SBC [R6_BANKED_ADDR_MOVED[2]],Y ; Sub from ptr.
...
VAL_LT_0x10: ; 1E:1C77, 0x03DC77
    ...
    LSR A
    LSR A
    JSR ENGINE_MULTIPLY_X/A_TO_UINT
    LSR A ; Nibble down.
    LSR A
    LSR A
    LSR A
...
X_STORE_LOOP?: ; 1E:1C97, 0x03DC97
    ...
    PHA ; Re-save it, too.
    TAY ; To stream index.
    LDA [RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY[8]],Y ; Load streamed.
    LDY #$10 ; Reseed with.
    ...
    JSR RANDOMIZE_GROUP_A ; Not set, do ??
    AND #$C0 ; Keep upper bits.
    BNE VAL_GTE_STREAM ; Either set, goto.
    LDA [R6_BANKED_ADDR_MOVED[2]],Y ; Load from stream.
    ORA LIB_LUT_BIT_TEST_0x80-0x01,X ; Set val.
...
SUB_PTR_SEED_TO_STREAM_INDEX_AND_BIT_TO_TEST: ; 1E:1CCD, 0x03DCCD
    ...
    TAX ; To X index.
    LDA LIB_LUT_BIT_TEST_0x80-0x01,X ; Load bit test from LUT.
    RTS ; Leave.
...
NO_OVERFLOW: ; 1E:1CE8, 0x03DCE8
    ASL A ; << 1, *2.
    ROL RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+1 ; Rotate into.
    ASL A ; 2x
    ROL RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+1
    ASL A ; 3x
    ROL RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+1
    CLC ; Prep add.
    ADC #$00 ; Do nothing, ew.
    STA RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY[8] ; Store addr L.
    LDA RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+1 ; Load ??
    AND #$07 ; Keep 0000.0111
    ADC #$98 ; Base, 0x98, to 9F.
    STA RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+1 ; Set PTR H,
...
DONE_FORWARD: ; 1E:1D2B, 0x03DD2B
    ...
    INC SAVE_ID_FOCUS/OTHER ; ++
    BNE LOOP_NONZERO ; != 0, goto.
    LDA FIRST_LAUNCHER_HOLD_FLAG? ; Load ??
    BNE EXIT_NO_WRITES ; != 0, goto.
    LDA SCRIPT_PAIR_PTR_B_SEED?[2] ; Load ??
    AND #$C0 ; Keep upper.
    ORA SOUND_MAIN_SONG_CURRENTLY_PLAYING_ID ; Set ??
...
LOOP_HELPER_A_LOT: ; 1E:1D72, 0x03DD72
    JSR BANK_R6_AND_LOAD_FILE_UNK ; Do.
    JSR ENGINE_WRAM_STATE_WRITEABLE ; Enable WRAM.
...
VAL_NONZERO: ; 1E:1D97, 0x03DD97
    ...
    JSR STREAM_UNK_RESERVATION_ATTRS_UNK ; Do ??
    LDY #$02 ; TODO this bullshit.
    LDA [SCRIPT_MAIN_FPTR[2]],Y ; Move file to file.
    AND #$3F ; Lower only.
    LDY #$15
    ...
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y
    LDY #$00
    LDA [SCRIPT_MAIN_FPTR[2]],Y ; Move top.
    ...
    LDY #$02
    LDA SCRIPT_MAIN_FPTR[2] ; Move same index.
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y
    INY ; Stream++
    LDA SCRIPT_MAIN_FPTR+1 ; Load ??
...
CLEAR_STREAM_LOOP: ; 1E:1E0C, 0x03DE0C
    ...
    RTS ; Leave.
STREAM_UNK_RESERVATION_ATTRS_UNK: ; 1E:1E13, 0x03DE13
    LDY #$00 ; Stream index.
    ...
    TAX ; To X.
    LDY #$08 ; Stream index.
    LDA SCRIPT_ATTRIBUTES_A,X ; Move attr to file.
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y
    LDY #$14 ; Stream index.
    LDA SCRIPT_ATTRIBUTE_B,X ; Move attr to file.
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y
    RTS ; Leave.
BANK_R6_AND_LOAD_FILE_UNK: ; 1E:1E29, 0x03DE29
    LDA SCRIPT_R6_ROUTINE_SELECT ; Load ??
    JSR BANK_HANDLER_R6_AND_BASE_FILE? ; Bank R6.
    ASL A ; Val based.
    TAX ; To index.
...
INIT_PTR_WRAM_6800/COUNT_UNK: ; 1E:1E3E, 0x03DE3E
    ...
    STX ENGINE_MAP_OBJ_RESERVATIONS/??+1
    LDX #$28
    STX SAVE_ID_FOCUS/OTHER ; Set count?
    RTS ; Leave.
BANKED_ADDR_TO_PTR_UNK: ; 1E:1E4B, 0x03DE4B
    LDY #$01 ; Stream index.
    LDA [R6_BANKED_ADDR_MOVED[2]],Y ; Load from stream.
    BEQ STREAM_ZERO ; == 0, leave.
    STA SCRIPT_MAIN_FPTR+1 ; Store ??
    DEY ; Stream--
    LDA [R6_BANKED_ADDR_MOVED[2]],Y ; Load from stream.
    STA SCRIPT_MAIN_FPTR[2] ; Store ??
    RTS ; Leave.
...
BANK_HANDLER_R6_AND_BASE_FILE?: ; 1E:1E6C, 0x03DE6C
    CMP #$2B ; If _ #$2B
    BCC LT_0x2B ; <, goto.
...
A_REBASED: ; 1E:1E82, 0x03DE82
    ...
    TXA ; Bank val to A.
    LDX #$06 ; Seed R6 mod.
    JSR ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Set R6 to bank.
...
STREAM_POSITIVE: ; 1E:1F12, 0x03DF12
    ...
    INY ; Stream++
    LDA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Load from stream.
    SBC LIB_BCD/EXTRA_FILE_BCD_B ; Sub with carry.
    STA LIB_BCD/EXTRA_FILE_BCD_B ; Store result.
    CLC ; Prep add.
    LDA SCRIPT_PAIR_PTR?[2] ; Load ??
    ADC #$A4 ; += 0xA4
    ...
    LDY #$06 ; Stream index.
    LDA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Load from file.
    SBC SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Mod val.
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Store.
    INY ; Stream++
    LDA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Load stream.
    SBC SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Mod val.
    STA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Store result.
    LDY #$08 ; Stream index.
    ...
    INY ; Stream++
    LDA LIB_BCD/EXTRA_FILE_BCD_A ; Load val.
    LSR LIB_BCD/EXTRA_FILE_BCD_B ; Shift down.
    ROR A ; Into A.
    LSR LIB_BCD/EXTRA_FILE_BCD_B ; 2x
    ROR A
    ...
    LSR SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; Shift down.
    ROR A ; Into A.
    LSR SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER+1 ; 2x
    ROR A
    STA [ENGINE_MAP_OBJ_RESERVATIONS/??[2]],Y ; Store at ptr.
    LDA ROUTINE_SWITCH_ID_TODO ; Load ??
    BMI VAL_NEGATIVE ; Negative, goto.
...
STREAM_NEG: ; 1E:1FAF, 0x03DFAF
    LDY RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+1 ; Load ??
    LDA SAVE_ID_FOCUS/OTHER ; Load ??
    EOR [RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+2],Y ; Invert at stream.
    BEQ INVERT_EQ_0x00 ; == 0, goto.
    RTS ; Leave if nonzero.
VAL_NEGATIVE: ; 1E:1FB8, 0x03DFB8
    LDY RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+1 ; Load ??
    LDA SAVE_ID_FOCUS/OTHER ; Load ?? for stream.
INVERT_EQ_0x00: ; 1E:1FBC, 0x03DFBC
    STA [RENAME_THIS_ARR_SOMETHING_SANE_EVENTUALLY+2],Y ; Store to stream.
    RTS ; Leave.
