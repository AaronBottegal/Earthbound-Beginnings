    ...
    STA **:$0100
    LDA #$03
    ORA ENGINE_MAPPER_CONFIG_STATUS
    STA MMC3_BANK_CFG
    LDX #$09
    LDY #$02
    DEY
...
DICTIONARY_ALT_VALS: ; 1E:022C, 0x03C22C
    ...
    JSR ENGINE_PACKET_CREATE_FROM_FILE
    STY NMI_PPU_CMD_PACKETS_INDEX
    ...
    JSR 1E:046E
    LDA #$00
    STA NMI_PPU_CMD_PACKETS_BUF[16],X
...
LOOP_LARGEST: ; 1E:054C, 0x03C54C
    JSR SUB_LOAD_UNK[Y]_RET_CC_0x01-0x05 ; Load val.
    BCS DATA_CS
    INC **:$6707 ; ++
...
VAL_LT_0x4: ; 1E:05C9, 0x03C5C9
    JSR SUB_LOAD_UNK[Y]_RET_CC_0x01-0x05 ; Do.
    BCS DATA_RET_OTHER
...
X_GTE_0x4: ; 1E:05EE, 0x03C5EE
    ...
    LDA #$C3 ; Move ??
    STA **:$670F
    JMP ENGINE_ENABLE_WRAM_NO_WRITES ; Exit, no more WRAM writes.
...
ROM_DATA_UNK_PAIR: ; 1E:061F, 0x03C61F
    ...
    .db F8
    .db 85
SUB_LOAD_UNK[Y]_RET_CC_0x01-0x05: ; 1E:062E, 0x03C62E
...
ENGINE_A_TO_UPDATE_PACKET: ; 1E:068B, 0x03C68B
    PHA ; Save A.
    JSR ENGINE_SETTLE_UPDATES? ; Settle.
    JSR PACKET_CALC_COORDS_TO_VADDR ; Calc screen pos.
    LDA #$05
    STA NMI_PPU_CMD_PACKETS_BUF[16] ; Set type, data upload +1 mode.
    LDA #$01 ; 1 byte.
...
ALT_ENTRY: ; 1E:06E8, 0x03C6E8
    ...
    LDA #$00 ; Clear val.
    STA NMI_PPU_CMD_PACKETS_BUF[16],Y ; EOF the packet.
    STA NMI_PPU_CMD_PACKETS_INDEX ; Reset index.
    LDA #$80
    STA NMI_FLAG_E5_TODO ; Set flag ??
...
LOOP_COUNT: ; 1E:075F, 0x03C75F
    LDA NMI_PPU_CMD_PACKETS_BUF[16],X ; Load from X.
...
BIT_0x40_CLEAR: ; 1E:07A4, 0x03C7A4
    ...
    JSR ENGINE_SETTLE_UPDATES?
    STX NMI_PPU_CMD_PACKETS_INDEX
    LDA NMI_PPU_CMD_PACKETS_BUF[16],X
    BEQ 1E:07FD
    EOR #$05
    BNE 1E:07F3
...
APPEND_DATA_TO_UPDATE_BUF: ; 1E:0A1F, 0x03CA1F
    STA NMI_PPU_CMD_PACKETS_BUF[16],X ; Store to arr.
    INX ; Index++
    TXA ; Index save.
...
PACKET_HEADER_HELPER_TO_EDGE_OF_SCREEN: ; 1E:0A3D, 0x03CA3D
    ...
    STA NMI_PPU_CMD_PACKETS_BUF[16],X ; Store to buf.
    INX ; Index++
    LDA #$00
    STA NMI_PPU_CMD_PACKETS_BUF[16],X ; Clear count.
    INX ; Index++
...
VAL_POSITIVE: ; 1E:0AA9, 0x03CAA9
    ...
    LDY #$00
    LDA [FPTR_PACKET_CREATION[2]],Y
    STA WORLD_POS?_CARRY_ADDS_UNK[3]
    INY
    LDA [FPTR_PACKET_CREATION[2]],Y
    STA WORLD_POS?_CARRY_ADDS_UNK+1
    INY
    LDA [FPTR_PACKET_CREATION[2]],Y
    STA WORLD_POS?_CARRY_ADDS_UNK+2
    PLA
    JSR ENGINE_SET_MAPPER_BANK_X_VAL_A
    JSR ENGINE_SETTLE_UPDATES?
    LDA #$0A
    STA NMI_PPU_CMD_PACKETS_BUF[16]
    LDA WORLD_POS?_CARRY_ADDS_UNK+2
    LSR A
    LDA WORLD_POS?_CARRY_ADDS_UNK+1
    ROR A
    SEC
    ROR A
    STA NMI_PPU_CMD_PACKETS_BUF+1
    ...
    ORA #$08
    STA NMI_PPU_CMD_PACKETS_BUF+2
    LDA WORLD_POS?_CARRY_ADDS_UNK[3]
    STA NMI_PPU_CMD_PACKETS_BUF+3
    LDA #$00
...
PTR_POSITIVE: ; 1E:0B21, 0x03CB21
    ...
    RTS ; Leave.
FORWARD_PACKET_FP_BY_A: ; 1E:0B38, 0x03CB38
    CLC ; Prep add.
...
SYSTEM_SETUP_COMPLETED: ; 1E:0B44, 0x03CB44
    JSR ENGINE_ENABLE_WRAM_NO_WRITES ; No writes.
    LDA #$C0
...
FLAG_20_EARLY_LOOP_UNK: ; 1E:0B57, 0x03CB57
    ...
    BEQ NONE_PRESSED
    JSR CTRL_BUTTONS_TO_ACTION_VAL?
    AND #$08 ; Test loaded.
...
VAL_NONZERO: ; 1E:0B91, 0x03CB91
    JSR ENGINE_SETTLE_UPDATES? ; Settle.
    LDA ENGINE_FLAG_20_SETUP_UNK
    BNE FLAG_20_EARLY_LOOP_UNK ; != 0, goto.
    JSR 1E:1D01 ; Do ??
    JSR 1E:1FDA
    JSR 1E:0C2B
    LDA **:$0021
    ...
    JSR ENGINE_HELPER_R7_0x13
    LDA #$00
    LDY CONTROL_ACCUMULATED?[2]
    STA CONTROL_ACCUMULATED?[2]
    LDA SCRIPT_FLAG_0x22
    ORA UNK_NONZERO_SKIP
    ORA **:$0021
    ORA ENGINE_FLAG_20_SETUP_UNK
    BNE RET_CC
    TYA
    AND #$F0
    BMI 1E:0BE2
...
RET_CC: ; 1E:0BEB, 0x03CBEB
    ...
    JSR 1E:1232
    LDA #$00
    STA NMI_PPU_CMD_PACKETS_BUF[16],X ; EOF.
    STA NMI_PPU_CMD_PACKETS_INDEX ; Reset index.
    LDA #$80
    STA NMI_FLAG_E5_TODO ; Set flag.
    BNE 1E:0C83 ; Always taken.
    JSR 1E:105E
    JSR 1E:10B1
    JSR 1E:1232
    LDA #$00
    ...
    INC **:$00D5
    RTS
CTRL_BUTTONS_TO_ACTION_VAL?: ; 1E:0C9D, 0x03CC9D
    LSR A ; Nibble down.
    LSR A
    LSR A
    LSR A
...
FILE_SELECTION_SCREEN_SETUP?: ; 1E:0E02, 0x03CE02
    ...
    LDX #$40 ; Count.
    STA NMI_PPU_CMD_PACKETS_BUF[16] ; Packet making.
...
LOOP_0x800: ; 1E:0E30, 0x03CE30
    ...
    STA NMI_FLAG_E5_TODO ; Set ??
    JSR ENGINE_SETTLE_UPDATES? ; Settle.
    JSR ENGINE_ENABLE_WRAM_WITH_WRITES ; Enable WRAM with writing.
    LDY #$00 ; Index.
LOOP_TO_TARGET: ; 1E:0E40, 0x03CE40
    ...
    JSR ENGINE_ENABLE_WRAM_NO_WRITES ; No WRAM writes now.
    CLC ; Prep add.
    TYA ; A = 0x40
...
ENGINE_LARGE_UNIQUE_PPU_PACKET_HELPER: ; 1E:0EB2, 0x03CEB2
    ...
    STY NMI_PPU_CMD_PACKETS_BUF[16] ; Y to, type.
    LDA #$40
...
INDEX_POSITIVE: ; 1E:0F32, 0x03CF32
    ...
    LDA SCRIPT_PAIR_UNK[2] ; Load.
    AND #$C0 ; Keep upper.
    STA SCRIPT_LOADED_SHIFTED_|VAL[2] ; Store it.
    LDA SCRIPT_PAIR_UNK+1 ; Move other.
...
LOOP_JMP: ; 1E:0F68, 0x03CF68
    ...
    SBC #$40 ; -= 0x40
    STA SCRIPT_LOADED_SHIFTED_UNK ; Store to.
    LDA **:$0019 ; Load ??
    SBC #$00 ; Carry sub.
    STA **:$00AB ; Store to.
    LDA #$13
    STA SCRIPT_COUNT_UNK ; Set ??
    JSR PTRS_A_LOT_IDFK ; Do ??
    DEC **:$009B ; --
...
VABIT_SET: ; 1E:0FFB, 0x03CFFB
    STA **:$0093 ; Store ??
    JSR ENGINE_SETTLE_UPDATES? ; Settle.
    JSR PACKET_CREATOR_HELPER_TODO ; Do.
    LDA #$00
    STA NMI_PPU_CMD_PACKETS_BUF[16],X ; End of packet.
    STA NMI_PPU_CMD_PACKETS_INDEX ; Reset index.
    LDA #$80
    STA NMI_FLAG_E5_TODO ; Set flag.
    DEC **:$009B ; --
...
TIMER_EQ_0x00: ; 1E:102E, 0x03D02E
    ...
    LDA #$04
    STA NMI_PPU_CMD_PACKETS_BUF[16] ; Set type, palette.
    LDA #$00
...
ROM_DATA_UNK: ; 1E:104E, 0x03D04E
    ...
    CLC
    LDA LUT_MOD_0x0+2,X
    ADC SCRIPT_PAIR_UNK[2]
    STA SCRIPT_PAIR_UNK[2]
    LDA LUT_MOD_0x0+3,X
    ADC SCRIPT_PAIR_UNK+1
    STA SCRIPT_PAIR_UNK+1
    CLC
    LDA LUT_MOD_0x0+2,X
    BEQ CREATE_VAL/INDEX_UNK
    BMI 1E:1094
    LDA **:$001D
    ADC #$20
    BCC 1E:109A
...
RTS: ; 1E:10C0, 0x03D0C0
    ...
    LDA **:$0018
    AND #$C0
    ADC EXTRA_SINGLE_TBL[2],X
    STA SCRIPT_LOADED_SHIFTED_UNK
    LDA **:$0019
    ADC EXTRA_SINGLE_TBL+1,X
    STA **:$00AB
    CLC
    LDA SCRIPT_PAIR_UNK[2]
    AND #$C0
    ADC ROM_DATA_ARR_UNK,X
    STA SCRIPT_LOADED_SHIFTED_|VAL[2]
    LDA SCRIPT_PAIR_UNK+1
    ADC SLOT_EXTRAS[4],X
    STA SCRIPT_LOADED_SHIFTED_|VAL+1
    LDA SLOT_EXTRAS+1,X
    STA **:$00AE
    ...
    JMP PTRS_A_LOT_IDFK
EXTRA_SINGLE_TBL: ; [2], 1E:10FD, 0x03D0FD
    .db C0
    .db FF
...
SLOT_EXTRAS: ; [4], 1E:1100, 0x03D100
    ...
    .db 10
    .db 00
PTRS_A_LOT_IDFK: ; 1E:111D, 0x03D11D
    JSR SETUP_UNK ; Do ??
...
REENTRY_NO_OVERFLOW: ; 1E:118F, 0x03D18F
    LDA SCRIPT_LOADED_SHIFTED_UNK ; Load ??
    LSR A ; >> 2, /4.
    LSR A
    ...
    JSR ENGINE_ENABLE_WRAM_NO_WRITES ; Disable WRAM.
    ...
    CLC ; Prep add.
    ADC SCRIPT_LOADED_SHIFTED_UNK ; Mod.
    STA SCRIPT_LOADED_SHIFTED_UNK ; Store result.
...
EXIT_RTS: ; 1E:121B, 0x03D21B
    ...
    LDA 1E:1289,X
    STA **:$0093
    JMP PACKET_CREATOR_HELPER_TODO
    .db 4C
    .db 98
    .db D3
    .db 00
    ...
    .db 10
    .db 30
PACKET_CREATOR_HELPER_TODO: ; 1E:12C4, 0x03D2C4
    JSR SCRIPT_R6/R7+?????
    LDA #$11
    STA **:$0091 ; Set ??
    LDX #$00 ; Index.
    LDA #$05
    STA NMI_PPU_CMD_PACKETS_BUF[16],X ; Type, 0x5. Single byte count times.
    STA **:$042A,X ; Set ??
    INX ; Index++
    LDA **:$0091
    ASL A
...
SCRIPT_R6/R7+?????: ; 1E:14B7, 0x03D4B7
    ...
    AND #$80
    LSR A
    STA SCRIPT_INVERT_UNK
    LSR A
    STA **:$0096
    BEQ 1E:1548
    LDA **:$0013
    BIT **:$01A5
    STA SCRIPT_B800_PTR_UNK+1
    LDA [STREAM_UNK_DEEP_A[2]],Y
    ASL A
    ASL A
    ROL SCRIPT_B800_PTR_UNK+1
    STA SCRIPT_B800_PTR_UNK[2]
    LDA SCRIPT_B800_PTR_UNK+1
    ADC #$90
    ADC **:$0096
    STA SCRIPT_B800_PTR_UNK+1
    RTS
SETUP_UNK: ; 1E:155D, 0x03D55D
    LDA **:$00AB ; Load.
...
SET_MULTIPLE?: ; 1E:15F3, 0x03D5F3
    LDA UNK_NONZERO_SKIP ; Load.
    BEQ VAL_ZERO ; == 0, goto.
    BPL VAL_POSITIVE ; Positive, goto.
    AND #$7F ; Keep lower.
    STA UNK_NONZERO_SKIP ; Store val to.
...
HUGE_ASS_STREAMS_THINGY_IDFK: ; 1E:1674, 0x03D674
    LDA ENGINE_FLAG_20_SETUP_UNK ; Load flag.
...
SETUP_POSITIVE: ; 1E:1697, 0x03D697
    ...
    STA SCRIPT_PAIR_UNK[2] ; Store to.
    ADC #$C0 ; += 0xC0
    STA SCRIPT_LOADED_SHIFTED_|VAL[2] ; Store to.
...
SETUP_NEG_COMPLETED: ; 1E:16CB, 0x03D6CB
    ...
    JSR ENGINE_ENABLE_WRAM_WITH_WRITES ; Enable with writes.
    LDA UNK_NONZERO_SKIP ; Load ??
...
VAL_EQ_0x00_CLOSE: ; 1E:170E, 0x03D70E
    LDA #$00
    STA ENGINE_FLAG_20_SETUP_UNK ; Clear flag.
    LDA SCRIPT_FLAG_0x22 ; Load.
    AND #$CF ; Keep 1100.1111
    STA SCRIPT_FLAG_0x22 ; Store back.
    JMP ENGINE_ENABLE_WRAM_NO_WRITES ; Goto, disable WRAM and leave.
...
STREAMS_MOD_UNK: ; 1E:1768, 0x03D768
    ...
    JSR MOD_STREAM_LUT_UNK ; Do.
    JSR ENGINE_ENABLE_WRAM_WITH_WRITES ; Enable WRAM.
    PLA ; Pull A/X passed.
    TAX
    PLA
    ...
    JMP ENGINE_ENABLE_WRAM_NO_WRITES ; Leave.
    LDX #$00
...
MOVE_TO_STREAM_UNK: ; 1E:17E2, 0x03D7E2
    LDY #$04 ; Stream index.
    LDA SCRIPT_LOADED_SHIFTED_UNK ; Move to stream.
    STA [ENGINE_FPTR_30[2]],Y
    INY ; Stream++
    LDA **:$00AB ; Load ??
    STA [ENGINE_FPTR_30[2]],Y
...
A_REBASED: ; 1E:1E82, 0x03DE82
    ...
    PLA ; Pull index for bank routine.
    RTS ; Leave.
    LDX #$06
    LDA #$00
    JMP ENGINE_SET_MAPPER_BANK_X_VAL_A
...
ENGINE_MAP_DISPLAY?: ; 1E:1E99, 0x03DE99
    JSR BANK_R6/INIT_PTR/SET_UNK ; Do.
    JSR ENGINE_ENABLE_WRAM_WITH_WRITES ; Enable writes.
...
STREAM_EQ_0x00: ; 1E:1EEF, 0x03DEEF
    ...
    STA [ENGINE_FPTR_30[2]],Y
    LDY STREAM_DEEP_INDEX
    LDA GAME_SLOT_CURRENT?
    STA [STREAM_DEEP_C],Y
    LDY ARR_BITS_TO_UNK+1
    LDA GAME_SLOT_CURRENT?
    EOR [ARR_BITS_TO_UNK+2],Y
    BEQ 1E:1FBC
    RTS
    LDY ARR_BITS_TO_UNK+1
    LDA GAME_SLOT_CURRENT?
    STA [ARR_BITS_TO_UNK+2],Y
    RTS
    LDY #$11
    LDA [ENGINE_FPTR_30[2]],Y
    STA STREAM_DEEP_INDEX
