    ...
    LDA #$80
    STA NMI_FLAG_EXECUTE_UPDATE_BUF_AND_MORE_TODO ; Set flag to run.
...
SHUTDOWN_COPY_PROTECTION: ; 19:01F8, 0x0321F8
    ...
    JSR SETTLE_SPRITES_OFFSCREEN/CLEAR_OBJ_EXTRA_MEMORY ; Init sprites/stuff.
...
LOOP_PALETTE: ; 19:024F, 0x03224F
    ...
    LDA #$80
    STA NMI_FLAG_EXECUTE_UPDATE_BUF_AND_MORE_TODO ; Set flag for updates.
...
SCRIPT_UNK_TODO: ; 19:02B3, 0x0322B3
    ...
    LDA #$10
    STA PACKET_CREATION_STREAM_INDEX ; Store consume?
...
LOWER_EQ_0x00: ; 19:030C, 0x03230C
    ...
    STA R_**:$00A5 ; Store back.
    DEC PACKET_CREATION_STREAM_INDEX ; Todo--
...
X_INC_0x4_LOOP: ; 19:0365, 0x032365
    ...
    RTS ; Leave.
ROUTINE_SEEDED_TODO: ; 19:036E, 0x03236E
...
INDEX_LT_0x4: ; 19:0373, 0x032373
    ...
    LDA #$FA ; Seed menu.
    LDX #$A3
    STA FPTR_MENU_PRIMARY[2] ; Set menu fptr.
    STX FPTR_MENU_PRIMARY+1
    JSR ENGINE_MENU_INIT_MASTER_FULL ; Init full.
...
VALUE_NEGATIVE: ; 19:047A, 0x03247A
    PLA ; Pull A, old R6.
    LDX #$06 ; R6
    JMP ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Set R6 with A, fancy exit.
...
OBJ_MOVE_UP_LOOP: ; 19:05E8, 0x0325E8
    LDA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],X ; Load lower.
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+32,X ; Store to offset, moving up 0x20.
    DEX ; Index--
...
INDEX_POSITIVE: ; 19:05F5, 0x0325F5
    ...
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+10
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+18
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+26
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Load.
    STA OBJ?_BYTE_0x3_UNK ; Set ??
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+11
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+19
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+27
    LDA #$5A
    ...
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+8
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+16
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS+24
...
OBJECTS_CREATE_DATA: ; 19:06A1, 0x0326A1
    ...
    LDA #$23
    STA SOUND_MAIN_SONG_INIT_ID ; Song init.
    LDA #$F8 ; Seed ??
    LDX #$FF
...
VAL_LT_0x20: ; 19:0764, 0x032764
    ...
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle.
    LDA #$30
    STA NMI_FLAG_EXECUTE_UPDATE_BUF_AND_MORE_TODO ; Update.
    RTS ; Leave.
...
INC_NO_OVERFLOW_C: ; 19:07EF, 0x0327EF
    ...
    RTS ; Leave.
BATTLE_HELPER_MATH_TODO: ; 19:07FC, 0x0327FC
...
DATA_NEGATIVE: ; 19:0867, 0x032867
    PLA ; R6 restore val.
    LDX #$06 ; R6.
    JMP ENGINE_MAPPER_COMMIT_BANK_X_VAL_A ; Goto.
COPY_PROTECT_FAILURE: ; 19:086D, 0x03286D
    JSR ENGINE_PALETTE_FADE_OUT ; Fade out.
    JSR ENGINE_0x300_OBJECTS_UNK? ; Do objects.
    JSR SETTLE_SPRITES_OFFSCREEN/CLEAR_OBJ_EXTRA_MEMORY
    JSR ENGINE_CLEAR_SCREENS_0x2000-0x2800 ; Clear screens.
    JSR ENGINE_SETTLE_ALL_UPDATES? ; Settle updates.
    LDA #$00
    STA NMI_LATCH_FLAG_UNK ; Clear flag.
...
MENU_POINTERS_H: ; 19:1886, 0x033886
    ...
    .db 12
    .db 37
WRAM_DATA_TO_EXTRA_UNK: ; 19:1A5B, 0x033A5B
    LDA #$04 ; Set ??
    STA WORLD_OBJECT_PAGE_EXTRA_ATTRS[256],Y
    LDA SAVE_GAME_MOD_PAGE_PTR/MATH_HELPER[2] ; Move ??
