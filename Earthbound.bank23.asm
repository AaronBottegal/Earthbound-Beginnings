...
STREAM_NE_0x8: ; 17:004A, 0x02E04A
    ...
    TYA ; Stream to A.
    PHA ; Save it.
    LDA ENGINE_PALETTE_FPTR/BITS/GEN_USE[2] ; Load moved.
    CMP #$FF ; If _ #$FF
...
COUNT_NONZERO: ; 17:0089, 0x02E089
    ...
    LDA #$8E ; Load base.
    ADC ENGINE_PALETTE_FPTR/BITS/GEN_USE+1 ; Add with val.
    STA ENGINE_PALETTE_FPTR/BITS/GEN_USE+1 ; Store result.
...
X_NE_0x8: ; 17:00D9, 0x02E0D9
    ...
    ASL A ; << 2, *4.
    ASL A
    ORA ENGINE_PALETTE_FPTR/BITS/GEN_USE[2] ; Combine with other.
    STA ENGINE_PALETTE_FPTR/BITS/GEN_USE[2] ; Store to.
    ASL A ; << 4, *16.
    ASL A
    ASL A
    ASL A
    ORA ENGINE_PALETTE_FPTR/BITS/GEN_USE[2] ; Combine with.
    ...
    LDA #$05
    STA NMI_PPU_CMD_PACKETS_BUF[64] ; Make packet TODO.
    LDA #$10
    STA NMI_PPU_CMD_PACKETS_BUF+1
    LDY #$00
    LDX #$04
Y_NE_0x8: ; 17:0110, 0x02E110
    ...
    STA FLAG_UNK_48
    LDX #$80
INDEX_NONZERE_LOOP: ; 17:013E, 0x02E13E
    TXA
    PHA
    LDY #$04
Y_NONZERO: ; 17:0142, 0x02E142
    LDA STREAM_INDEXES_ARR_UNK+4,X
    AND #$03
    STA STREAM_INDEXES_ARR_UNK+4,X
    INX
    INX
    DEY ; Y--
    BNE Y_NONZERO ; != 0, loop.
    PLA
    CLC
    ADC #$20
    TAX
    BNE INDEX_NONZERE_LOOP ; != 0, goto.
...
SUB_UNK_PTRS_ARR_UNK: ; 17:022D, 0x02E22D
    ...
    LDA ENGINE_PALETTE_FPTR/BITS/GEN_USE[2] ; Move from 0x60 to 0x62
    STA ENGINE_PALETTE_FPTR/BITS/GEN_USE+2
    LDA #$00 ; Init.
    LDX #$05 ; Shifts to do.
X_NONZERO: ; 17:023E, 0x02E23E
    ASL ENGINE_PALETTE_FPTR/BITS/GEN_USE+2 ; << 1 var.
    ROL A ; Into A.
    DEX ; X--
    ...
    CLC ; Prep add.
    LDA #$00
    ADC ENGINE_PALETTE_FPTR/BITS/GEN_USE+2 ; += val.
    STA ENGINE_PALETTE_FPTR/BITS/GEN_USE+2 ; Store result, same.
    LDA #$80 ; Val adding.
...
STREAM_NE_0x18: ; 17:0257, 0x02E257
    LDA [ENGINE_PALETTE_FPTR/BITS/GEN_USE+2],Y ; Load from file 62
    ...
    LDA ENGINE_PALETTE_FPTR/BITS/GEN_USE+2 ; Move to arr.
    STA STREAM_PTRS_ARR_UNK[48],X
